...
Boosts have one finite-score safety exception to the pinned grammar (DIV-005): a syntactically valid boost that cannot be represented as a finite non-negative `f32` is diagnosed as `InvalidBoost` and recovered as the unboosted branch; its parser syntax key likewise treats the boost as absent. Individually finite nested boosts whose cumulative product becomes non-finite fail deterministically during lowering. Exact Tantivy score-bit parity excludes two registered classes: this non-finite class, and the summation-association envelope (DIV-007). Quill fuses each unfielded term's `[content, 2.0×title]` expansion into one summed contribution per term while the pinned grammar accumulates two interleaved clauses per term, so composite queries (eight-plus leaves, or a boost over the multi-field expansion, or unspliceable mixed-occur nesting) may differ by at most 2 ULP with identical result sets — a bounded, typed tolerance (`ScoreEpsilon`/`SummationAssociation`, owner ruling bd-55mvg), never a membership change. All other shapes, including pure-disjunctive nesting spliced by the Should-flatten, remain bit-exact.
