...
| Stale sidecar hazard (found by the watch proof) | a watcher's shutdown compaction rewrote `index.fsvi` and left the one-shot `.fec` in place; the next `fsfs compact` would have restored the pre-compaction bytes (data loss opened by the protector wiring above) | the index crate drops `<fsvi>.fec` on every main-file write (compaction, vacuum, finish, in-place tombstones); publishers re-protect, otherwise doctor says unprotected | index test `main_file_writes_remove_the_durability_sidecar`; real binary after a watched create: both generations unprotected until the next compact (e9d1525b) |
| Watch mode quality tier (bd-orb50) | with the default storage catalog a watched create/update enqueued only the fast embedding job; the quality tier fell behind and REFINED never ranked the new content (v1.7.0 too) | the storage-backed branch writes the quality vector inline | real binary, isolated catalog: `gamma.md` lexical 0 and semantic 0 after exit; both tiers compacted to 3 records; doctor quality generation at 3 live documents (e9d1525b) |
| Global catalog scoping (bd-3tym7, new) | the default catalog is keyed by relative path across index roots, so a file already known under root A is "unchanged" under root B and never gets vectors in watch mode | bead filed with three options; proofs use a per-run catalog | reproduced twice before isolating the catalog |
| x86_64 macOS build (found by the release) | the generation-root host checks (a1990de7) gated their macOS helpers and the manifest's `rustix` dependency on arm64 while the callers were unconditional; the lite x86_64 macOS asset could not be built | gates widened to all of macOS in source and manifest (test-only arm64 gates kept) | mmini cross build compiles and packages `fsfs-lite-1.8.0-x86_64-apple-darwin` (cfde499f, 66341808) |
| **Release v1.8.0 (Gap #1)** | v1.7.0 users had none of the above | tag `v1.8.0` at 66341808; `scripts/quality-gate.sh` 7/7 on that commit (2026-09-02T09:26Z: fmt 5 s, check 19 s, clippy 14 s, workspace lib tests 83 s, fsfs test binaries 118 s, real-model e2e 40 s, quick-start 157 s); 13 assets, the v1.7.0 set (lite musl x2 built with zigbuild here, lite/full darwin built natively on mmini, full gnu here, per-file `.sha256`, `SHA256SUMS`) | `install.sh --lite` and the default glibc route both install `fsfs 1.8.0`; the v1.7.0 lite binary's `fsfs update` downloads the musl asset, verifies the checksum, backs up, and reports 1.8.0. Two earlier gate runs on the candidates were 6/7: e9d1525b lost the fsfs stage to two watcher tests that time out under the concurrent release builds (both pass alone), d02302e2 was 7/7 before the sidecar fix moved the commit |
