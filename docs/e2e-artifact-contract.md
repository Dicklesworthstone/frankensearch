...
| Performance regression artifact contract (`bd-2hz.9.6`) | `provisional_artifact_only` | CI quality lane + benchmark comparison artifacts | Required artifact-schema check only: policy/result artifacts (`perf_regression_gate_policy.json`, `perf_regression_gate_result.json`) emit `gate_active: false`, `no_decision_provisional`, and a replay command; they are not a regression verdict | Promote to an active gate only after benchmark harness inputs and evaluation logic are wired for all required lanes. |
