...
| 2026-06-26 | BlackThrush | **Shared-tree contention blocks clean `simd.rs` commits** + ready-to-land `f32_f32` 4-acc lever | Identified the next dot-kernel lever: `dot_product_f32_f32_unchecked` is still **single-accumulator** and on **real paths** — WAL-entry scoring (`search.rs:528,546`, `two_tier.rs:310,392,415`) and **MRL truncated search** (`mrl.rs:464,497`). It is cheap-decode (plain f32 load), so the 4-accumulator ILP pattern that won for `f32_bytes` (ledger) and `dot_i8_i8` (`0.856`/`0.939`) should win here too (~1.1–1.2×). **Could not land it:** the shared working tree had **uncommitted concurrent WIP in `simd.rs`** (an in-flight 8-accumulator `dot_i8_i8` escalation, +19/-11 vs HEAD, NOT on origin) and in `dot_product.rs`, so `git add simd.rs` would capture another agent's work and `pull --rebase` is blocked by it. Verified that WIP compiles + passes all 24 dot/int8 tests (incl. `dot_i8_i8_matches_scalar`), so it is correct, just unfinished. | **Blocker = no file coordination across agents in one working dir.** Whoever next owns `simd.rs` (uncontended): apply 4 accumulators to `dot_product_f32_f32_unchecked` (bench A/B already exists as `f32_slice_new`/`f32_slice_old`); verify no f32 bit-identity test breaks (4 accs reorder the float sum — the same accepted trade as `f32_bytes`). Fleet fix: isolated git worktrees per agent, or `file_reservation` (agent-mail) on `simd.rs`/`dot_product.rs`/ledgers before editing. |
