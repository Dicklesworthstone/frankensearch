...
The lockfile census also found 46 transitive update units. This pass includes
the Rustls security update and the Crossbeam Channel, Crossbeam Utils and
Zerocopy soundness updates; 42 routine transitive units remain deferred while
the requested release is qualified. This is a complete direct-dependency
refresh, not a claim that every transitive package is at its latest release.
...
The publication contract now requires the same audited 0.4.4 identity for the
family and pager. Its negative probes retain rejection of the old family,
pager 0.4.0/0.4.3, mixed versions, Git sources and unreviewed pager 0.4.5.
This advances the exact accepted dependency identity; it does not waive any
runtime, platform or release gate.
...
golden is changed. Through RCH on `vmi1152480`, manifest history passes 126
tests with no failures and one existing ignore. The existing MiniLM, Snowflake
and Nomic real-model certificates each pass in a fresh process, without any
certificate changes. The executed binary SHA-256 is
`598e23d5100d0fae94de2f5bfd6ef8355d51fa7f2c6ac3c6b33511125984af5a`.
The receipt and hash-verified logs are retained under
`dependencies/fastembed701-stage-evidence/`. The RCH cross-build also passes
native execution on `Mac-mini-max`: 126 manifest-history tests, one existing
ignore, and all three unchanged macOS certificates in separate processes.
The executed Mach-O SHA-256 is
...
Locked metadata resolution and RCH compilation of both fuzz binaries pass.
The receipt is `dependencies/refresh-queue-fuzz-stage-evidence/receipt.json`.
This is a compile check, not a fuzz campaign.

### Combined compiler gates and queue follow-through

Formatting, workspace all-target checks, Clippy with warnings denied,
hybrid-feature Clippy and the Windows index compile guard pass through RCH.
The gate exposed five lints in the recently committed queue lease work:
explicit guard drops, a `must_use` annotation and the standard no-op waker
