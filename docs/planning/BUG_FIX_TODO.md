...
- [ ] Complete `scripts/quality-gate.sh` remotely with registered models; retain failures instead of weakening gates.
  ...
  - Current corrected doctor regression f17e0a73 is queued for the same warmed ovh target with a finite 7200-second transport budget, unchanged model-test deadline, and retained source/executable/output files under `.rch-results/doctor-producer-20260912-f17e0a73`. Initial receipt wrapper was blocked before execution for dynamic redirect targets; explicit new paths replace them without disabling the guard.
