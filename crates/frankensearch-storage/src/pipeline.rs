...
mod tests {
    ...
    fn process_batch_unknown_embedder_fails_job_without_aborting_batch() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            // A claim now carries the document identity it was queued for, and
            // a failure for a revision the catalog no longer holds is
            // suppressed as stale. Queue the unknown tier for the stored
            // revision, so its failure is the unknown embedder's alone.
            let content_hash = runner
                .storage
                .get_document("doc-unknown-embedder")
                .expect("document lookup")
                .expect("ingested document")
                .content_hash;
            let queued_unknown = runner
                .queue
                .enqueue("doc-unknown-embedder", "missing-tier", &content_hash, 0)
