...
/// Work lanes governed by resource-admission policy.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ResourceLane {
    /// Phase-1 search path that must stay available under resource pressure.
    InitialSearch,
    /// Expensive phase-2 quality refinement or reranking.
    QualityRefinement,
    /// Indexing and embedding work that can be paced behind query service.
    Indexing,
    /// Watcher refresh work generated by filesystem changes.
    WatchRefresh,
    /// Model-load or model-warmup work.
    ModelLoad,
}
...
impl ResourceLane {
    ...
    const fn priority(self) -> u8 {
        match self {
            Self::InitialSearch => 0,
            Self::QualityRefinement => 1,
            Self::Indexing => 2,
            Self::WatchRefresh => 3,
            Self::ModelLoad => 4,
        }
...
/// Admission outcome for one resource-governed lane.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ResourceAdmissionMode {
    /// Start immediately.
    Admit,
    /// Start with a reduced budget or pacing delay.
    Throttle,
    /// Queue for a later retry.
    Defer,
    /// Reject until an operator-controlled condition changes.
    Reject,
}

impl ResourceAdmissionMode {
    const fn admitted(self) -> bool {
        matches!(self, Self::Admit | Self::Throttle)
    }
}

/// asupersync-aware backpressure guidance for callers.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResourceBackpressure {
    pub yield_before_start: bool,
    pub cancel_on_pressure_escalation: bool,
    pub retry_after_ms: u64,
    pub queue_budget: u16,
}

/// Resource pressure signals beyond the host-pressure controller.
#[derive(Debug, Clone, Copy, PartialEq, Serialize, Deserialize)]
pub struct ResourcePressureInput {
    pub timestamp_ms: u64,
    pub host_pressure: PressureSignal,
    pub host_state: PressureState,
    pub model_load_pct: f64,
    pub index_write_pct: f64,
...
impl ResourcePressureInput {
    #[must_use]
    pub fn normalized(self) -> Self {
        Self {
            host_pressure: PressureSignal::new(
                self.host_pressure.cpu_pct,
                self.host_pressure.memory_pct,
                self.host_pressure.io_pct,
                self.host_pressure.load_pct,
            ),
            model_load_pct: normalize_pct(self.model_load_pct),
            index_write_pct: normalize_pct(self.index_write_pct),
            ..self
        }
    }
}

/// Configuration for deterministic resource-admission decisions.
#[derive(Debug, Clone, Copy, PartialEq, Serialize, Deserialize)]
pub struct ResourcePressureGovernorConfig {
    pub model_load_defer_pct: f64,
    pub index_write_throttle_pct: f64,
    pub queue_constrained_depth: u16,
    pub queue_degraded_depth: u16,
    pub retry_after_constrained_ms: u64,
    pub retry_after_degraded_ms: u64,
    pub retry_after_emergency_ms: u64,
    pub max_queue_budget: u16,
}

impl Default for ResourcePressureGovernorConfig {
    fn default() -> Self {
        Self {
            model_load_defer_pct: 70.0,
            index_write_throttle_pct: 75.0,
            queue_constrained_depth: 8,
            queue_degraded_depth: 32,
            retry_after_constrained_ms: 25,
            retry_after_degraded_ms: 100,
            retry_after_emergency_ms: 250,
            max_queue_budget: 64,
        }
    }
}

impl ResourcePressureGovernorConfig {
    /// # Errors
    ///
    /// Returns [`SearchError::InvalidConfig`] when admission thresholds or
    /// queue budgets are outside valid ranges.
    pub fn validate(self) -> SearchResult<Self> {
        if !(1.0..=200.0).contains(&self.model_load_defer_pct) {
            return Err(SearchError::InvalidConfig {
                field: "resource_governor.model_load_defer_pct".to_owned(),
                value: self.model_load_defer_pct.to_string(),
                reason: "must be in [1, 200]".to_owned(),
            });
        }
        if !(1.0..=200.0).contains(&self.index_write_throttle_pct) {
            return Err(SearchError::InvalidConfig {
                field: "resource_governor.index_write_throttle_pct".to_owned(),
                value: self.index_write_throttle_pct.to_string(),
                reason: "must be in [1, 200]".to_owned(),
            });
        }
        if self.queue_constrained_depth == 0 {
            return Err(SearchError::InvalidConfig {
                field: "resource_governor.queue_constrained_depth".to_owned(),
                value: "0".to_owned(),
                reason: "must be >= 1".to_owned(),
            });
        }
        if self.queue_degraded_depth < self.queue_constrained_depth {
            return Err(SearchError::InvalidConfig {
                field: "resource_governor.queue_degraded_depth".to_owned(),
                value: self.queue_degraded_depth.to_string(),
                reason: "must be >= queue_constrained_depth".to_owned(),
            });
        }
        if self.max_queue_budget == 0 {
            return Err(SearchError::InvalidConfig {
                field: "resource_governor.max_queue_budget".to_owned(),
                value: "0".to_owned(),
                reason: "must be >= 1".to_owned(),
            });
        }
...
/// Deterministic resource-admission decision for one lane.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResourceAdmissionDecision {
    pub timestamp_ms: u64,
    pub lane: ResourceLane,
    pub lane_priority: u8,
    pub mode: ResourceAdmissionMode,
    pub admitted: bool,
    pub effective_pressure_state: PressureState,
    pub effective_degradation_stage: DegradationStage,
    pub reason_code: String,
    pub backpressure: ResourceBackpressure,
    pub operator_advice: String,
    pub metric_key: String,
}

/// Resource pressure governor for fsfs work-lane admission.
#[derive(Debug, Clone, Default)]
pub struct ResourcePressureGovernor {
    config: ResourcePressureGovernorConfig,
}

pub const REASON_RESOURCE_INITIAL_SEARCH_ADMITTED: &str = "degrade.admit.initial_search";
pub const REASON_RESOURCE_LANE_ADMITTED: &str = "degrade.admit.lane";
pub const REASON_RESOURCE_QUALITY_DEFERRED: &str = "degrade.defer.quality";
pub const REASON_RESOURCE_MODEL_LOAD_DEFERRED: &str = "degrade.defer.model_load";
pub const REASON_RESOURCE_INDEX_THROTTLED: &str = "degrade.throttle.index_write";
pub const REASON_RESOURCE_WATCH_THROTTLED: &str = "degrade.throttle.watch_refresh";
pub const REASON_RESOURCE_PRESSURE_DEFERRED: &str = "degrade.defer.pressure";
pub const REASON_RESOURCE_HARD_PAUSE: &str = "degrade.pause.hard_pause";

impl ResourcePressureGovernor {
    /// # Errors
    ///
    /// Returns [`SearchError::InvalidConfig`] when `config` is invalid.
    pub fn new(config: ResourcePressureGovernorConfig) -> SearchResult<Self> {
        Ok(Self {
            config: config.validate()?,
        })
    }
    ...
    #[must_use]
    pub const fn config(&self) -> ResourcePressureGovernorConfig {
        self.config
    }
    ...
    pub fn decide(
        &self,
        input: ResourcePressureInput,
        lane: ResourceLane,
    ) -> ResourceAdmissionDecision {
        let input = input.normalized();
        let effective_state = self.effective_pressure_state(input);
        let stage = Self::effective_degradation_stage(input, effective_state);

        if input.hard_pause_requested {
            return self.decision(
                input,
                lane,
                ResourceAdmissionMode::Reject,
                effective_state,
                stage,
                REASON_RESOURCE_HARD_PAUSE,
                "Clear the hard pause before admitting fsfs work.",
            );
        }

        match lane {
            ResourceLane::InitialSearch => self.decision(
                input,
                lane,
                ResourceAdmissionMode::Admit,
                effective_state,
                stage,
                REASON_RESOURCE_INITIAL_SEARCH_ADMITTED,
                "Keep phase-1 search admitted; shed slower lanes first.",
            ),
            ResourceLane::QualityRefinement => {
                if input.quality_circuit_open
                    || input.model_load_pct >= self.config.model_load_defer_pct
                    || effective_state.severity() >= PressureState::Constrained.severity()
                {
                    self.decision(
                        input,
                        lane,
                        ResourceAdmissionMode::Defer,
                        effective_state,
                        stage,
                        REASON_RESOURCE_QUALITY_DEFERRED,
                        "Defer quality refinement and return initial results.",
                    )
                } else {
                    self.allowed(input, lane, effective_state, stage)
                }
            }
            ResourceLane::Indexing => {
                if effective_state == PressureState::Emergency {
                    self.decision(
                        input,
                        lane,
                        ResourceAdmissionMode::Defer,
                        effective_state,
                        stage,
                        REASON_RESOURCE_PRESSURE_DEFERRED,
                        "Hold indexing until emergency pressure clears.",
                    )
                } else if input.index_write_pct >= self.config.index_write_throttle_pct
                    || effective_state.severity() >= PressureState::Constrained.severity()
                {
                    self.decision(
                        input,
                        lane,
                        ResourceAdmissionMode::Throttle,
                        effective_state,
                        stage,
                        REASON_RESOURCE_INDEX_THROTTLED,
                        "Throttle index writes and preserve search latency.",
                    )
                } else {
                    self.allowed(input, lane, effective_state, stage)
                }
            ...
            ResourceLane::WatchRefresh => {
                if effective_state.severity() >= PressureState::Degraded.severity() {
                    self.decision(
                        input,
                        lane,
                        ResourceAdmissionMode::Defer,
                        ...
                        stage,
                        REASON_RESOURCE_PRESSURE_DEFERRED,
                        "Defer watcher refresh work while pressure is degraded.",
                    )
                } else if effective_state == PressureState::Constrained {
                    self.decision(
                        input,
                        lane,
                        ResourceAdmissionMode::Throttle,
                        effective_state,
                        stage,
                        REASON_RESOURCE_WATCH_THROTTLED,
                        "Pace watcher refreshes with cancellation-aware backpressure.",
                    )
                } else {
                    self.allowed(input, lane, effective_state, stage)
                }
            ...
            ResourceLane::ModelLoad => {
                if input.model_load_pct >= self.config.model_load_defer_pct
                    || effective_state.severity() >= PressureState::Constrained.severity()
                {
                    self.decision(
                        input,
                        ...
                        effective_state,
                        stage,
                        REASON_RESOURCE_MODEL_LOAD_DEFERRED,
                        "Defer model loading until query-critical lanes recover.",
                    )
                } else {
                    self.allowed(input, lane, effective_state, stage)
                }
    ...
    #[must_use]
    pub fn decide_all(&self, input: ResourcePressureInput) -> Vec<ResourceAdmissionDecision> {
        ResourceLane::ORDERED
            .iter()
            .copied()
            .map(|lane| self.decide(input, lane))
            .collect()
    }
    ...
    fn allowed(
        ...
        self.decision(
            ...
            effective_state,
            stage,
            REASON_RESOURCE_LANE_ADMITTED,
            "Admit lane with normal structured-concurrency cancellation.",
        )
    }
    ...
    fn decision(
        ...
        ResourceAdmissionDecision {
            ...
            mode,
            admitted: mode.admitted(),
            effective_pressure_state: effective_state,
            effective_degradation_stage: stage,
            reason_code: reason_code.to_owned(),
            backpressure: self.backpressure(mode, effective_state, lane),
            operator_advice: operator_advice.to_owned(),
            metric_key: format!("fsfs.resource_admission.{lane:?}.{mode:?}").to_ascii_lowercase(),
        }
    }
    ...
    fn backpressure(
        ...
        let retry_after_ms = match effective_state {
            PressureState::Normal => 0,
            PressureState::Constrained => self.config.retry_after_constrained_ms,
            PressureState::Degraded => self.config.retry_after_degraded_ms,
            PressureState::Emergency => self.config.retry_after_emergency_ms,
        };
        let state_budget = match effective_state {
            PressureState::Normal => self.config.max_queue_budget,
            PressureState::Constrained => self.config.max_queue_budget / 2,
            PressureState::Degraded => self.config.max_queue_budget / 4,
            PressureState::Emergency => 1,
        };
        ResourceBackpressure {
            yield_before_start: mode != ResourceAdmissionMode::Admit
                || effective_state.severity() >= PressureState::Constrained.severity(),
            cancel_on_pressure_escalation: lane != ResourceLane::InitialSearch
                && effective_state.severity() >= PressureState::Constrained.severity(),
            retry_after_ms: if mode == ResourceAdmissionMode::Admit {
                0
            } else {
                retry_after_ms
            },
            queue_budget: state_budget.max(1),
        }
    }

    fn effective_pressure_state(&self, input: ResourcePressureInput) -> PressureState {
        [
            input.host_state,
            Self::signal_state(input.host_pressure.score()),
            self.model_load_state(input.model_load_pct),
            self.index_write_state(input.index_write_pct),
            self.queue_state(input.queued_work, input.active_agents),
        ]
        .into_iter()
    ...
    fn effective_degradation_stage(
        input: ResourcePressureInput,
        pressure_state: PressureState,
    ) -> DegradationStage {
        DegradationSignal::new(
            pressure_state,
            input.quality_circuit_open,
            input.hard_pause_requested,
        )
        .auto_target_stage()
    }

    fn signal_state(score: f64) -> PressureState {
        PressureThresholds::for_profile(PressureProfile::Performance).state_for_up(score)
    }

    fn model_load_state(&self, model_load_pct: f64) -> PressureState {
        if model_load_pct >= self.config.model_load_defer_pct + 75.0 {
            PressureState::Emergency
        } else if model_load_pct >= self.config.model_load_defer_pct + 35.0 {
            PressureState::Degraded
        } else if model_load_pct >= self.config.model_load_defer_pct {
    ...
    fn index_write_state(&self, index_write_pct: f64) -> PressureState {
        if index_write_pct >= self.config.index_write_throttle_pct + 75.0 {
            PressureState::Emergency
        } else if index_write_pct >= self.config.index_write_throttle_pct + 35.0 {
            PressureState::Degraded
        } else if index_write_pct >= self.config.index_write_throttle_pct {
            PressureState::Constrained
        } else {
            PressureState::Normal
        }
    ...
    fn queue_state(&self, queued_work: u16, active_agents: u16) -> PressureState {
        let agent_weighted_queue = queued_work.saturating_add(active_agents.saturating_mul(2));
        if agent_weighted_queue >= self.config.queue_degraded_depth.saturating_mul(2) {
            PressureState::Emergency
        } else if agent_weighted_queue >= self.config.queue_degraded_depth {
            PressureState::Degraded
        } else if agent_weighted_queue >= self.config.queue_constrained_depth {
            PressureState::Constrained
        } else {
            PressureState::Normal
        }
...
mod tests {
    ...
    fn resource_input(
        host_state: PressureState,
        host_pressure: PressureSignal,
    ) -> ResourcePressureInput {
        ResourcePressureInput {
            timestamp_ms: 10,
            host_pressure,
            host_state,
    ...
    #[test]
    fn resource_governor_never_starves_initial_search_under_pressure() {
        let governor = ResourcePressureGovernor::default();
        let mut input = resource_input(
            PressureState::Emergency,
            PressureSignal::new(180.0, 170.0, 160.0, 150.0),
        );
        input.model_load_pct = 200.0;
        input.index_write_pct = 200.0;
        input.active_agents = 12;
        input.queued_work = 96;

        let decision = governor.decide(input, ResourceLane::InitialSearch);

        assert!(decision.admitted);
        assert_eq!(decision.mode, ResourceAdmissionMode::Admit);
        assert_eq!(
            decision.reason_code,
            REASON_RESOURCE_INITIAL_SEARCH_ADMITTED
        );
        assert_eq!(
            decision.effective_degradation_stage,
    ...
    #[test]
    fn resource_governor_defers_quality_and_model_load_under_model_pressure() {
        let governor = ResourcePressureGovernor::default();
        let mut input = resource_input(
            PressureState::Normal,
            PressureSignal::new(20.0, 20.0, 10.0, 10.0),
        );
        input.model_load_pct = 120.0;

        let quality = governor.decide(input, ResourceLane::QualityRefinement);
        let model_load = governor.decide(input, ResourceLane::ModelLoad);

        assert_eq!(quality.mode, ResourceAdmissionMode::Defer);
        assert!(!quality.admitted);
        assert_eq!(quality.reason_code, REASON_RESOURCE_QUALITY_DEFERRED);
        assert!(quality.backpressure.cancel_on_pressure_escalation);
        assert_eq!(model_load.mode, ResourceAdmissionMode::Defer);
        assert_eq!(model_load.reason_code, REASON_RESOURCE_MODEL_LOAD_DEFERRED);
    }

    #[test]
    fn resource_governor_throttles_index_and_watch_under_shared_pressure() {
        let governor = ResourcePressureGovernor::default();
        let mut input = resource_input(
            PressureState::Normal,
            PressureSignal::new(25.0, 25.0, 20.0, 20.0),
        );
        input.index_write_pct = 90.0;
        input.active_agents = 4;

        let indexing = governor.decide(input, ResourceLane::Indexing);
        let watch = governor.decide(input, ResourceLane::WatchRefresh);

        assert_eq!(indexing.mode, ResourceAdmissionMode::Throttle);
        assert!(indexing.admitted);
        assert_eq!(indexing.reason_code, REASON_RESOURCE_INDEX_THROTTLED);
        assert!(indexing.backpressure.yield_before_start);
        assert_eq!(watch.mode, ResourceAdmissionMode::Throttle);
        assert_eq!(watch.reason_code, REASON_RESOURCE_WATCH_THROTTLED);
    }

    #[test]
    fn resource_governor_hard_pause_rejects_all_lanes() {
        let governor = ResourcePressureGovernor::default();
        let mut input = resource_input(
            PressureState::Normal,
            PressureSignal::new(10.0, 10.0, 5.0, 5.0),
        );
        input.hard_pause_requested = true;

        for decision in governor.decide_all(input) {
            assert_eq!(decision.mode, ResourceAdmissionMode::Reject);
            assert!(!decision.admitted);
            assert_eq!(decision.reason_code, REASON_RESOURCE_HARD_PAUSE);
            assert_eq!(
                decision.effective_degradation_stage,
    ...
    #[test]
    fn resource_governor_decides_lanes_in_deterministic_fair_order() {
        let governor = ResourcePressureGovernor::default();
        let input = resource_input(
            PressureState::Constrained,
            PressureSignal::new(75.0, 35.0, 20.0, 30.0),
        );

        let decisions = governor.decide_all(input);
        let lanes: Vec<_> = decisions.iter().map(|decision| decision.lane).collect();
        let priorities: Vec<_> = decisions
            .iter()
            .map(|decision| decision.lane_priority)
            .collect();

        assert_eq!(
            lanes,
            vec![
                ResourceLane::InitialSearch,
                ResourceLane::QualityRefinement,
                ResourceLane::Indexing,
                ResourceLane::WatchRefresh,
                ResourceLane::ModelLoad,
        ...
        assert!(
            decisions
                .iter()
                .all(|decision| !decision.operator_advice.is_empty())
        );
    }

    #[test]
    fn resource_governor_rejects_invalid_config() {
        let err = ResourcePressureGovernor::new(ResourcePressureGovernorConfig {
            queue_constrained_depth: 8,
            queue_degraded_depth: 4,
            ..ResourcePressureGovernorConfig::default()
        })
        .expect_err("invalid queue ordering");
