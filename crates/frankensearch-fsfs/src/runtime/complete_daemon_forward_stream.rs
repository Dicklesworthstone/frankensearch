...
use super::{
    FSFS_DAEMON_CLIENT_IO_TIMEOUT_MS, FSFS_DAEMON_RESPONSE_MAX_BYTES, FsfsRuntime,
    LiveRetainedSearchReader, PeerOutcome, Reply, Request, SearchExecutionFlags, VERSION,
    codec_error, complete_cli_error, control, decode_reply, decode_request, encode_response,
    error_envelope, make_request, retained_search_checkpoint, run_request, validate_request,
    write_response,
};
...
pub(super) fn reported_error(error: &SearchError) -> Option<&OutputError> {
    match error {
        SearchError::SubsystemError { source, .. } => {
            source.downcast_ref::<ReportedFailure>().map(|failure| &failure.0)
        }
        _ => None,
    }
}
...
fn parse_request(bytes: &[u8]) -> SearchResult<StreamRequest> {
    ...
    if request.fsfs_complete_cli_stream != VERSION || request.request.search.query.trim().is_empty() {
        return Err(complete_cli_error("daemon_stream", "unsupported streaming CLI protocol"));
    }
...
pub(super) async fn serve(
    ...
    let producer = async {
        ...
        query_runtime.cli_input.filter.clone_from(&request.request.search.filter);
        ...
        query_runtime.run_search_stream_command_with_writer(
            cx,
...
impl ReceiveState {
    fn accept(&mut self, frame: Frame, request: &Request) -> SearchResult<Option<Frame>> {
        ...
        {
            return Err(complete_cli_error("daemon_stream", "invalid stream identity, ordering or trailing frame"));
        }
        match &frame.event {
            StreamEvent::Started(started) => {
                ...
                {
                    return Err(complete_cli_error("daemon_stream", "invalid or duplicate Started frame"));
                }
                self.started = true;
            }
            _ if !self.started => {
                return Err(complete_cli_error("daemon_stream", "stream did not begin with Started"));
            }
            StreamEvent::Terminal(terminal) => {
                let coherent = match terminal.status {
                    StreamTerminalStatus::Completed => terminal.exit_code == 0 && terminal.error.is_none(),
                    StreamTerminalStatus::Failed | StreamTerminalStatus::Cancelled => {
                        (1..=255).contains(&terminal.exit_code)
                            && terminal.error.as_ref().is_some_and(|error| error.exit_code == terminal.exit_code)
                    }
                };
                if !coherent {
                    return Err(complete_cli_error("daemon_stream", "inconsistent terminal status or error"));
                }
            ...
            StreamEvent::Result(result) if result.rank == 0 => {
                return Err(complete_cli_error("daemon_stream", "result rank must be positive"));
            }
            _ => {}
        }
        ...
        self.next_sequence = frame.seq.checked_add(1).ok_or_else(|| {
            complete_cli_error("daemon_stream", "stream sequence exhausted")
        })?;
        Ok(Some(frame))
    }

    fn emit<W: Write>(&mut self, mut frame: Frame, format: OutputFormat, output: &mut W) -> SearchResult<()> {
...
async fn receive<W: Write + Send>(
    ...
    loop {
        ...
        match peer.read(&mut chunk) {
            Ok(0) => {
                if !record.is_empty() {
                    return Err(io::Error::new(ErrorKind::UnexpectedEof, "partial daemon stream record").into());
                }
                let terminal = state.terminal.take().ok_or_else(|| {
                    SearchError::Io(io::Error::new(ErrorKind::UnexpectedEof, "daemon stream closed without Terminal"))
                })?;
                let error = match &terminal.event {
                    StreamEvent::Terminal(terminal) => terminal.error.clone(),
                    _ => return Err(complete_cli_error("daemon_stream", "missing terminal state")),
                };
            ...
            Ok(count) => {
                for part in chunk[..count].split_inclusive(|byte| *byte == b'\n') {
                    ...
                    if !state.started && serde_json::from_slice::<serde_json::Value>(&record)
                        .is_ok_and(|value| value.get("fsfs_complete_cli").is_some())
                    {
                        return match decode_reply(&record, request) {
                            Err(error) => Err(error),
                            Ok(_) => Err(complete_cli_error("daemon_stream", "buffered success cannot satisfy a stream request")),
                        };
                    }
...
fn append_record(record: &mut Vec<u8>, part: &[u8]) -> SearchResult<()> {
    if part.len() > FSFS_DAEMON_RESPONSE_MAX_BYTES.saturating_sub(record.len()) {
        return Err(complete_cli_error("daemon_stream", "stream record exceeds response bound"));
    }
...
impl FsfsRuntime {
    ...
    pub(crate) async fn stream_complete_generation_daemon<W: Write + Send>(
        ...
        retained_search_checkpoint(cx)?;
        if self.cli_input.expand || !matches!(self.cli_input.format, OutputFormat::Jsonl | OutputFormat::Toon) {
            return Err(complete_cli_error("daemon_options", "stream forwarding requires JSONL/TOON without expansion"));
        }
        ...
        if bytes.len() > FSFS_DAEMON_REQUEST_MAX_BYTES {
            return Err(complete_cli_error("daemon_request", "request exceeds its byte limit"));
        }
        parse_request(&bytes)?;
        ...
        let result = receive(cx, &mut peer, &request.request, started, timeout,
            self.cli_input.format, &mut state, output).await;
        finish_receive(&mut state, &request.request, self.cli_input.format, output, result)
    }
}
...
mod tests {
    ...
    fn request() -> Request {
        ...
        make_request(&FsfsRuntime::new(FsfsConfig::default()), root.path(), "query", 10).unwrap()
    }

    fn started(request: &Request) -> Frame {
        Frame::new(&request.request_id, 0, "2026-09-21T00:00:00Z", "search",
            StreamEvent::Started(StreamStartedEvent::new(&request.request_id, &request.search.query, "jsonl")))
    }

    fn completed(request: &Request) -> Frame {
        Frame::new(&request.request_id, 1, "2026-09-21T00:00:00Z", "search",
    ...
    async fn read(
        cx: &Cx, request: &Request, bytes: &[u8], format: OutputFormat,
    ) -> (Vec<u8>, SearchResult<()>) {
        ...
        let result = receive(cx, &mut peer, request, Instant::now(), Duration::from_secs(1),
            format, &mut state, &mut output).await;
    ...
    fn streaming_request_is_versioned_nested_and_preserves_filter_and_explain() {
        ...
        let wrapper = StreamRequest { fsfs_complete_cli_stream: VERSION, request, explain: Some(true) };
    ...
    fn frame_identity_sequence_and_started_are_checked_before_delivery() {
        let request = request();
        for frame in [
            Frame { stream_id: "wrong request".into(), ..started(&request) },
            Frame { seq: u64::MAX, ..started(&request) },
            Frame { v: 99, ..started(&request) },
            Frame { ts: String::new(), ..started(&request) },
            Frame { seq: 0, ..completed(&request) },
        ] {
            assert!(ReceiveState::default().accept(frame, &request).is_err());
        }
        let mut state = ReceiveState::default();
        state.accept(started(&request), &request).unwrap();
        assert!(state.accept(Frame { seq: 2, ..completed(&request) }, &request).is_err());
        let duplicate = Frame { seq: 1, ..started(&request) };
        assert!(state.accept(duplicate, &request).is_err());
        let mut offset = ReceiveState::default();
        offset.accept(Frame { seq: 7, ..started(&request) }, &request).unwrap();
        assert!(offset.accept(Frame { seq: 8, ..completed(&request) }, &request).unwrap().is_none());
    }
    ...
    fn terminal_validation_keeps_the_public_protocol_retry_contract() {
        ...
        assert!(state.accept(completed(&request), &request).unwrap().is_none());
    }
    ...
    fn eof_without_terminal_replaces_truncation_with_one_failed_terminal() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            let frames: Vec<Frame> = output.split(|byte| *byte == b'\n').filter(|line| !line.is_empty())
    ...
    fn terminal_success_is_not_exposed_when_the_peer_appends_another_frame() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            let request = request();
            let extra = Frame { seq: 2, ..started(&request) };
    ...
    fn remote_failed_terminal_retains_its_exit_status_without_a_second_envelope() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            let terminal = Frame::new(&request.request_id, 1, "2026-09-21T00:00:00Z", "search",
                StreamEvent::Terminal(terminal_event_from_error(&original, 0, 0)));
            let bytes = frames_bytes(&[started(&request), terminal]);
            let (output, result) = read(&cx, &request, &bytes, OutputFormat::Jsonl).await;
            let error = result.unwrap_err();
            assert_eq!(output, bytes);
            assert_eq!(FsfsRuntime::forwarded_search_error(&error).unwrap().exit_code,
    ...
    fn cancellation_and_expired_deadline_emit_nothing_before_started() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            let result = receive(&cx, &mut peer, &request, Instant::now(), Duration::from_secs(1),
                OutputFormat::Jsonl, &mut state, &mut output).await;
            assert!(matches!(result, Err(SearchError::Cancelled { .. })));
            cx.set_cancel_requested(false);
            let result = receive(&cx, &mut peer, &request, Instant::now(), Duration::ZERO,
                OutputFormat::Jsonl, &mut state, &mut output).await;
    ...
    fn cancellation_after_started_emits_one_cancelled_terminal() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            cx.set_cancel_requested(true);
            let result = receive(
                &cx, &mut peer, &request, Instant::now(), Duration::from_secs(1),
                OutputFormat::Jsonl, &mut state, &mut output,
            ).await;
            let error = finish_receive(
                &mut state, &request, OutputFormat::Jsonl, &mut output, result,
            ).unwrap_err();
            cx.set_cancel_requested(false);
            let frames: Vec<Frame> = output.split(|byte| *byte == b'\n')
                .filter(|line| !line.is_empty())
        ...
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            let server = std::thread::spawn(move || {
                ...
                peer.write_all(&frames_bytes(&[
                    started(&request.request), completed(&request.request),
                ])).unwrap();
            });
            ...
            let mut output = Vec::new();
            let result = runtime.run_complete_generation_search_with_writer(
                &cx, &root, &mut output,
            ).await;
            server.join().unwrap();
            result.unwrap();
    ...
    fn broken_output_is_never_written_again_by_error_reporting() {
        ...
        let error = state.emit(started(&request), OutputFormat::Jsonl, &mut output).unwrap_err();
        let writes = output.0;
        let error = finish_receive(&mut state, &request, OutputFormat::Jsonl, &mut output, Err(error)).unwrap_err();
...
mod generation_tests {
    ...
    fn forwarded_stream_uses_real_retained_retrieval_and_keeps_the_bundle_intact() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            serve(&cx, &runtime, &mut session, &mut peer,
                &serde_json::to_vec(&request).unwrap(), Duration::from_secs(5)).await.unwrap();
            drop(peer);
            let mut state = ReceiveState::default();
            let mut output = Vec::new();
            let result = receive(&cx, &mut client, &request.request, Instant::now(),
                Duration::from_secs(5), OutputFormat::Jsonl, &mut state, &mut output).await;
            finish_receive(&mut state, &request.request, OutputFormat::Jsonl, &mut output, result).unwrap();
            let frames: Vec<Frame> = output.split(|byte| *byte == b'\n').filter(|line| !line.is_empty())
                .map(|line| serde_json::from_slice(line).unwrap()).collect();
            assert!(matches!(&frames.first().unwrap().event, StreamEvent::Started(_)));
            assert_eq!(frames.iter().filter(|frame| matches!(&frame.event, StreamEvent::Result(_))).count(), 1);
            assert!(matches!(&frames.last().unwrap().event, StreamEvent::Terminal(terminal)
