...
mod tests {
    ...
    /// E6.3 input-order law runner. The stable external document IDs are the
    /// precondition; every returned observable is projected through the normal
    /// differential harness so this does not compare an invented side channel.
    #[cfg(feature = "perf-harness")]
    async fn e63_input_order_observations(
        cx: &Cx,
        documents: &[frankensearch_core::IndexableDocument],
        cases: &[(&str, &str)],
        seed: u64,
        ...
        for &(case_id, query) in cases {
            let mut case = DifferentialCase::new(format!("e63-input-order-{case_id}"), query, 16);
            case.snippet_max_chars = None;
            case.tie_expansion_limit = 64;
            case.metadata.generator_id = Some("e6.3-input-order-permutation-v1".to_owned());
            case.metadata.generator_seed = Some(seed);
            let run = harness
                .run(cx, &subject, &oracle, &case)
                .await
                .unwrap_or_else(|error| panic!("E6.3 input-order case {case_id} failed: {error}"));
            assert_eq!(
                run.comparison.status,
                ComparisonStatus::Exact,
                "E6.3 input-order cross-engine case {case_id}: {:?}",
    ...
    fn e63_seeded_input_order_permutation_is_exact_and_content_mutation_is_not() {
        ...
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            let baseline = e63_input_order_observations(&cx, &canonical, &queries, SEED).await;
            let transformed = e63_input_order_observations(&cx, &permuted, &queries, SEED).await;
            let invalid = e63_input_order_observations(&cx, &content_mutated, &queries, SEED).await;
