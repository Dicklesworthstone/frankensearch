...
mod tests {
    ...
    /// E6.3 lifecycle probe: the public lexical upsert path and an explicit
    /// delete followed by a fresh add should expose the same final live row.
    /// This is deliberately a full-observation probe: if replacement history
    /// changes BM25 scores or the published live-ID projection, the transform
    /// is rejected rather than reduced to a membership-only assertion.
    #[cfg(feature = "perf-harness")]
    #[test]
    fn e63_upsert_matches_delete_then_add_for_one_replacement() {
        use frankensearch_core::{IndexableDocument, LexicalSearch};

        const SEED: u64 = 0xe63_a053_7add_0001;
        let original = IndexableDocument::new("replace-id", "alpha original");
        let replacement = IndexableDocument::new("replace-id", "beta replacement");
        let invalid = IndexableDocument::new("replace-id", "gamma invalid");
        let queries = [
            ("replacement", "beta"),
            ("original", "alpha"),
            ("negative", "saffron"),
        ...
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            let mut upsert_subject = qg_position_mode_subject(true);
            upsert_subject
                .claim_fresh_campaign()
                .expect("claim E6.3 upsert subject");
            upsert_subject
                .index_mut()
                .expect("open E6.3 upsert subject")
                .index_documents(&cx, std::slice::from_ref(&original))
                .await
                .expect("index E6.3 upsert original");
            upsert_subject
                .index_mut()
                .expect("open E6.3 upsert subject")
                .commit(&cx)
                .await
                .expect("commit E6.3 upsert original");
            upsert_subject
                .mark_committed()
                .expect("publish E6.3 upsert original");
            upsert_subject
                .index()
                .expect("E6.3 upsert index")
                .index_document(&cx, &replacement)
                .await
                .expect("upsert E6.3 replacement");

            let mut delete_add_subject = qg_position_mode_subject(true);
            delete_add_subject
                .claim_fresh_campaign()
                .expect("claim E6.3 delete-add subject");
            delete_add_subject
                .index_mut()
                .expect("open E6.3 delete-add subject")
                .index_documents(&cx, std::slice::from_ref(&original))
                .await
                .expect("index E6.3 delete-add original");
            delete_add_subject
                .index_mut()
                .expect("open E6.3 delete-add subject")
                .commit(&cx)
                .await
                .expect("commit E6.3 delete-add original");
            delete_add_subject
                .mark_committed()
                .expect("publish E6.3 delete-add original");
            assert!(
                delete_add_subject
                    .index()
                    .expect("E6.3 delete-add index")
                    .delete_document(&cx, "replace-id")
                    .await
                    .expect("delete E6.3 original"),
                "E6.3 delete-add fixture must remove the original live ID"
            );
            delete_add_subject
                .index()
                .expect("E6.3 delete-add index")
                .index_document(&cx, &replacement)
                .await
                .expect("add E6.3 replacement after delete");

            let mut invalid_subject = qg_position_mode_subject(true);
            invalid_subject
                .claim_fresh_campaign()
                .expect("claim E6.3 invalid subject");
            invalid_subject
                .index_mut()
                .expect("open E6.3 invalid subject")
                .index_documents(&cx, std::slice::from_ref(&original))
                .await
                .expect("index E6.3 invalid original");
            invalid_subject
                .index_mut()
                .expect("open E6.3 invalid subject")
                .commit(&cx)
                .await
                .expect("commit E6.3 invalid original");
            invalid_subject
                .mark_committed()
                .expect("publish E6.3 invalid original");
            invalid_subject
                .index()
                .expect("E6.3 invalid index")
                .index_document(&cx, &invalid)
                .await
                .expect("upsert E6.3 invalid replacement");

            for (case_id, query) in queries {
                let mut case = DifferentialCase::new(format!("e63-upsert-{case_id}"), query, 16);
                case.snippet_max_chars = None;
                case.tie_expansion_limit = 64;
                case.metadata.generator_id = Some("e6.3-upsert-delete-add-v1".to_owned());
                case.metadata.generator_seed = Some(SEED);
                let upsert = upsert_subject
                    .observe(&cx, &case)
                    .await
                    .expect("observe E6.3 upsert result");
                let delete_add = delete_add_subject
                    .observe(&cx, &case)
                    .await
                    .expect("observe E6.3 delete-add result");
                let comparison =
                    compare_observations(upsert.clone(), delete_add, ComparatorConfig::default())
                        .expect("compare E6.3 upsert/delete-add observations");
                assert_eq!(
                    comparison.status,
                    ComparisonStatus::Exact,
                    "E6.3 upsert/delete-add must preserve the total observation for {case_id}: {:?}",
                    comparison.divergences,
                );

                if case_id == "replacement" {
                    let invalid_observation = invalid_subject
                        .observe(&cx, &case)
                        .await
                        .expect("observe E6.3 invalid replacement");
                    let invalid_comparison = compare_observations(
                        upsert,
                        invalid_observation,
                        ComparatorConfig::default(),
                    )
                    .expect("compare E6.3 invalid replacement");
                    assert_eq!(
                        invalid_comparison.status,
                        ComparisonStatus::Failed,
                        "E6.3 content mutation must not be accepted as a replacement transform"
                    );
                }
            }
        });
