...
// the git dependency.
const ORACLE_V4_TANTIVY_VERSION: &str = "0.26.1";
...
// v5: the gh-39 facade release (b31fa58f) bumped the oracle wrapper to
// frankensearch-lexical 0.2.3. Audit of the wrapper between the v4 mint
// (4329dad8) and that revision: Cargo.toml version only, plus assertion-style
// and lint-drift edits in lib.rs (bf4c6f5e, 099faccd) — no contract surface
// change. The registry tantivy artifact is byte-identical to v4 (same
// checksum). v4 joins v2/v3 as a retained decode-only line: historical
// artifacts stay verifiable, but nothing new admits under the 0.2.2 wrapper.
pub const CURRENT_ORACLE_TANTIVY_VERSION: &str = "0.26.2";
const ORACLE_V5_TANTIVY_VERSION: &str = "0.26.1";
const ORACLE_V5_TANTIVY_SOURCE: &str = "registry+https://github.com/rust-lang/crates.io-index";
const ORACLE_V5_TANTIVY_CHECKSUM_SHA256: &str =
    "edde6a10743fff00a4e1a8c9ef020bf5f3cbad301b7d2d39f2b07f123c4eac07";
...
const ORACLE_V6_TANTIVY_VERSION: &str = "0.26.1";
...
// v7 (fsfs 1.10.0 / crates 0.5.0): lexical 0.2.5 adds writer-free read-only
// opening, typed write refusal for those handles, and a diagnostic parse-tree
// accessor. The shipping parser, scoring and oracle observation bodies remain
// unchanged from e6394bc8; ordinary/oracle constructors still create writers.
// sha2 is now optional but remains enabled by tantivy-oracle. The exact registry
// Tantivy artifact is unchanged. The audit binds the final wrapper bytes at
// 1b67c0cb; v6 remains exact decode-only evidence and cannot admit new runs.
const ORACLE_V7_TANTIVY_VERSION: &str = "0.26.1";
const ORACLE_V7_TANTIVY_SOURCE: &str = "registry+https://github.com/rust-lang/crates.io-index";
const ORACLE_V7_TANTIVY_CHECKSUM_SHA256: &str =
    "edde6a10743fff00a4e1a8c9ef020bf5f3cbad301b7d2d39f2b07f123c4eac07";
const ORACLE_V7_LEXICAL_PACKAGE: &str = "frankensearch-lexical";
const ORACLE_V7_LEXICAL_PACKAGE_VERSION: &str = "0.2.5";
const ORACLE_V7_LEXICAL_CONTRACT_AUDIT_REVISION: &str = "1b67c0cb5cdeb6edeb8fa945b52da4d877215bef";
// v8 updates the registry oracle to Tantivy 0.26.2. The wrapper observation
// surface is unchanged; the new artifact checksum and domain distinguish new
// runs from every retained 0.26.1 record. v7 remains exact decode-only evidence.
const ORACLE_V8_TANTIVY_VERSION: &str = "0.26.2";
const ORACLE_V8_TANTIVY_SOURCE: &str = "registry+https://github.com/rust-lang/crates.io-index";
const ORACLE_V8_TANTIVY_CHECKSUM_SHA256: &str =
    "861facfabd71044968f364837f9a083b56464ba5a59079f88706ee5c451ca069";
const ORACLE_V8_LEXICAL_PACKAGE: &str = "frankensearch-lexical";
const ORACLE_V8_LEXICAL_PACKAGE_VERSION: &str = "0.2.5";
const ORACLE_V8_LEXICAL_CONTRACT_AUDIT_REVISION: &str = "1b67c0cb5cdeb6edeb8fa945b52da4d877215bef";
// v9 binds the 0.3.0 wrapper used by the Asupersync 0.5 release. Comparing
// 1b67c0cb with 347be73e changes only the wrapper package version; its source
// and observation contract are unchanged. v8 remains exact archival evidence.
const ORACLE_V9_LEXICAL_PACKAGE_VERSION: &str = "0.3.0";
const ORACLE_V9_LEXICAL_CONTRACT_AUDIT_REVISION: &str = "347be73ebad6d0172df99ec78c394e2095ece8a4";
// v10 binds the 0.3.1 release wrapper. The lexical crate diff from 347be73e
// to cf000487 changes only Cargo.toml's package version; its observation
...
const ORACLE_V2_DEPENDENCY_CONTRACT_HASH_DOMAIN: &[u8] =
...
const ORACLE_V7_DEPENDENCY_CONTRACT_HASH_DOMAIN: &[u8] =
    b"frankensearch/quill/oracle-dependency-contract/v7\0";
const ORACLE_V8_DEPENDENCY_CONTRACT_HASH_DOMAIN: &[u8] =
    b"frankensearch/quill/oracle-dependency-contract/v8\0";
const ORACLE_V9_DEPENDENCY_CONTRACT_HASH_DOMAIN: &[u8] =
...
pub struct OracleVersionContract {
    ...
    pub tantivy_source: String,
    pub tantivy_checksum_sha256: String,
    pub lexical_package: String,
    pub lexical_package_version: String,
    /// Historical revision where the wrapper contract was independently
    /// audited. This is not the source identity of the executing producer.
    pub lexical_contract_audit_revision: String,
}
...
impl OracleVersionContract {
    ...
    /// # Errors
    ///
    /// Returns an error when any current package, source, checksum, or
    /// historical audit pin is malformed or changed.
    ...
    fn validate_structure(&self, permit_retained_historical: bool) -> Result<(), GauntletError> {
        ...
        let matches_v4 = self.schema_version == 4
            && self.tantivy_version == ORACLE_V4_TANTIVY_VERSION
        ...
        let matches_v5 = self.schema_version == 5
            && self.tantivy_version == ORACLE_V5_TANTIVY_VERSION
            && self.tantivy_source == ORACLE_V5_TANTIVY_SOURCE
            && self.tantivy_checksum_sha256 == ORACLE_V5_TANTIVY_CHECKSUM_SHA256
        ...
        let matches_v6 = self.schema_version == 6
            && self.tantivy_version == ORACLE_V6_TANTIVY_VERSION
        ...
        let matches_v7 = self.schema_version == 7
            && self.tantivy_version == ORACLE_V7_TANTIVY_VERSION
            && self.tantivy_source == ORACLE_V7_TANTIVY_SOURCE
            && self.tantivy_checksum_sha256 == ORACLE_V7_TANTIVY_CHECKSUM_SHA256
            && self.lexical_package == ORACLE_V7_LEXICAL_PACKAGE
            && self.lexical_package_version == ORACLE_V7_LEXICAL_PACKAGE_VERSION
            && self.lexical_contract_audit_revision == ORACLE_V7_LEXICAL_CONTRACT_AUDIT_REVISION;
        let matches_v8 = self.schema_version == 8
            && self.tantivy_version == ORACLE_V8_TANTIVY_VERSION
            ...
            && self.lexical_package_version == ORACLE_V8_LEXICAL_PACKAGE_VERSION
            && self.lexical_contract_audit_revision == ORACLE_V8_LEXICAL_CONTRACT_AUDIT_REVISION;
        let matches_v9 = self.schema_version == 9
            && self.tantivy_version == ORACLE_V8_TANTIVY_VERSION
            && self.tantivy_source == ORACLE_V8_TANTIVY_SOURCE
            && self.tantivy_checksum_sha256 == ORACLE_V8_TANTIVY_CHECKSUM_SHA256
            && self.lexical_package == ORACLE_V8_LEXICAL_PACKAGE
            && self.lexical_package_version == ORACLE_V9_LEXICAL_PACKAGE_VERSION
            && self.lexical_contract_audit_revision == ORACLE_V9_LEXICAL_CONTRACT_AUDIT_REVISION;
        let matches_v10 = self.schema_version == 10
            ...
            || !is_lower_hex(&self.lexical_contract_audit_revision, 40)
        {
            return Err(GauntletError::InvalidContract {
                reason: "stored oracle dependency contract is malformed or internally inconsistent"
                    .to_owned(),
            });
        }
        Ok(())
    ...
    pub(crate) fn validate_current_dependency(&self) -> Result<(), GauntletError> {
        ...
        if self.tantivy_version != LOCKED_TANTIVY_VERSION
            || self.tantivy_source != LOCKED_TANTIVY_SOURCE
            || self.tantivy_checksum_sha256 != LOCKED_TANTIVY_CHECKSUM_SHA256
            || self.lexical_package_version != FRANKENSEARCH_LEXICAL_CRATE_VERSION
        {
            return Err(GauntletError::InvalidContract {
                reason: "embedded oracle dependency contract does not match the exact Cargo.lock resolution"
                    .to_owned(),
            });
        }
        #[cfg(feature = "tantivy-oracle")]
        if self.lexical_package_version
            != frankensearch_lexical::FRANKENSEARCH_LEXICAL_CRATE_VERSION
        {
            return Err(GauntletError::InvalidContract {
                reason: "embedded oracle wrapper version does not match the linked frankensearch-lexical crate"
                    .to_owned(),
            });
        }
        Ok(())
    ...
    /// Domain-separated identity of the self-contained dependency record.
    pub(crate) fn identity_sha256(&self) -> Result<String, GauntletError> {
        self.validate_retained_structure()?;
        let hash_domain = match self.schema_version {
            ...
            7 => ORACLE_V7_DEPENDENCY_CONTRACT_HASH_DOMAIN,
...
pub fn oracle_version_contract() -> Result<OracleVersionContract, GauntletError> {
    ...
    contract.validate_current_dependency()?;
    Ok(contract)
}
...
async fn run_q1_concat_merge_fixture(cx: &Cx) -> Result<Vec<String>, GauntletError> {
    ...
    let mut index = QuillIndex::in_memory(config.clone())
...
mod tests {
    ...
    #[test]
    fn embedded_oracle_contract_matches_exact_lock_resolution() {
        let contract = oracle_version_contract().expect("valid oracle contract");
        assert_eq!(contract.tantivy_version, LOCKED_TANTIVY_VERSION);
        assert_eq!(contract.tantivy_source, LOCKED_TANTIVY_SOURCE);
        assert_eq!(
            contract.tantivy_checksum_sha256,
            LOCKED_TANTIVY_CHECKSUM_SHA256
        );
        assert_eq!(
        ...
        contract
            ...
            .expect("stored v10 contract is self-contained");
        contract
    ...
    #[test]
    fn stored_oracle_contract_rejects_every_mutated_field() {
        let contract = oracle_version_contract().expect("valid oracle contract");
        ...
        let mut mutated = contract.clone();
        // 0.26.1 is the current version, so a version mutation must move
        ...
        mutations.push(mutated);
        let mut mutated = contract.clone();
        mutated.tantivy_checksum_sha256 = "0".repeat(64);
        mutations.push(mutated);
        let mut mutated = contract.clone();
        mutated.lexical_package = "lookalike-lexical".to_owned();
        mutations.push(mutated);
        let mut mutated = contract.clone();
        mutated.lexical_package_version = "0.2.0".to_owned();
        mutations.push(mutated);
        let mut mutated = contract;
        mutated.lexical_contract_audit_revision = "f".repeat(40);
        mutations.push(mutated);

        for mutation in mutations {
            assert!(
                mutation.validate_stored_structure().is_err(),
                "mutated stored dependency contract was accepted: {mutation:?}"
            );
        }
    }

    #[test]
    fn retained_v2_oracle_contract_is_exact_but_never_current() {
        let retained = OracleVersionContract {
            schema_version: 2,
            tantivy_version: ORACLE_V2_TANTIVY_VERSION.to_owned(),
            tantivy_source: ORACLE_V2_TANTIVY_SOURCE.to_owned(),
            tantivy_checksum_sha256: ORACLE_V2_TANTIVY_CHECKSUM_SHA256.to_owned(),
            lexical_package: ORACLE_V2_LEXICAL_PACKAGE.to_owned(),
            lexical_package_version: ORACLE_V2_LEXICAL_PACKAGE_VERSION.to_owned(),
            lexical_contract_audit_revision: ORACLE_V2_LEXICAL_CONTRACT_AUDIT_REVISION.to_owned(),
        };

        retained
            .validate_retained_structure()
            .expect("the committed v2 dependency record remains decode-only evidence");
        assert!(retained.validate_stored_structure().is_err());
        assert!(retained.validate_current_dependency().is_err());
        assert_eq!(
            retained.identity_sha256().expect("retained v2 identity"),
            "58988c117389e3222fc6c2202d311fc70158ef0e65e069100181be5fd98d3ba3",
            "retained evidence must keep the v2 domain used by committed ledger records",
        );
    ...
    #[test]
    fn retained_v4_oracle_contract_is_exact_but_never_current() {
        let retained = OracleVersionContract {
            schema_version: 4,
            tantivy_version: ORACLE_V4_TANTIVY_VERSION.to_owned(),
            tantivy_source: ORACLE_V4_TANTIVY_SOURCE.to_owned(),
            tantivy_checksum_sha256: ORACLE_V4_TANTIVY_CHECKSUM_SHA256.to_owned(),
            lexical_package: ORACLE_V4_LEXICAL_PACKAGE.to_owned(),
            lexical_package_version: ORACLE_V4_LEXICAL_PACKAGE_VERSION.to_owned(),
            lexical_contract_audit_revision: ORACLE_V4_LEXICAL_CONTRACT_AUDIT_REVISION.to_owned(),
        };

        retained
            .validate_retained_structure()
            .expect("the committed v4 dependency record remains decode-only evidence");
        assert!(retained.validate_stored_structure().is_err());
        assert!(retained.validate_current_dependency().is_err());
        assert_eq!(
            retained.identity_sha256().expect("retained v4 identity"),
            "b4f07a5bb537796cd0b201d807df10110d55542cfd6b2a923094bbbeff6db52d",
            "retained evidence must keep the v4 domain used by committed ledger records",
        );

        // The only difference between v4 and v5 is the wrapper line; a v4
        // record relabelled with the v5 wrapper version is neither.
        let mut substituted = retained;
        substituted.lexical_package_version = ORACLE_V5_LEXICAL_PACKAGE_VERSION.to_owned();
        assert!(substituted.validate_retained_structure().is_err());
        assert!(substituted.identity_sha256().is_err());
    }
    ...
    fn retained_v5_oracle_contract_is_exact_but_never_current() {
        let retained = OracleVersionContract {
            schema_version: 5,
            tantivy_version: ORACLE_V5_TANTIVY_VERSION.to_owned(),
    ...
    #[test]
    fn retained_v6_oracle_contract_is_exact_but_never_current() {
        let retained = OracleVersionContract {
            schema_version: 6,
            tantivy_version: ORACLE_V6_TANTIVY_VERSION.to_owned(),
            tantivy_source: ORACLE_V6_TANTIVY_SOURCE.to_owned(),
            tantivy_checksum_sha256: ORACLE_V6_TANTIVY_CHECKSUM_SHA256.to_owned(),
            lexical_package: ORACLE_V6_LEXICAL_PACKAGE.to_owned(),
            lexical_package_version: ORACLE_V6_LEXICAL_PACKAGE_VERSION.to_owned(),
            lexical_contract_audit_revision: ORACLE_V6_LEXICAL_CONTRACT_AUDIT_REVISION.to_owned(),
        };
        retained
            .validate_retained_structure()
            .expect("the committed v6 dependency record remains decode-only evidence");
        assert!(retained.validate_stored_structure().is_err());
        assert!(retained.validate_current_dependency().is_err());
        assert_eq!(
            retained.identity_sha256().expect("retained v6 identity"),
            "523d8a96a58584eb254981ca315c05a4d4d99e07733d7306802b6c2835f9a3a1",
            "retained evidence must keep the exact v6 identity",
        );

        let mut encoded = serde_json::to_value(retained).expect("serialize retained v6");
        for (field, value) in [
            ("schema_version", serde_json::json!(7)),
            ("tantivy_version", serde_json::json!("0.27.0")),
            (
                "tantivy_source",
                serde_json::json!("git+https://example.invalid/tantivy"),
            ),
            ("tantivy_checksum_sha256", serde_json::json!("0".repeat(64))),
            ("lexical_package", serde_json::json!("lookalike-lexical")),
            (
                "lexical_package_version",
                serde_json::json!(ORACLE_V7_LEXICAL_PACKAGE_VERSION),
            ),
            (
                "lexical_contract_audit_revision",
                serde_json::json!(ORACLE_V7_LEXICAL_CONTRACT_AUDIT_REVISION),
            ),
        ] {
            let original = encoded[field].clone();
            encoded[field] = value;
            let mutated: OracleVersionContract =
                serde_json::from_value(encoded.clone()).expect("well-typed mutation");
            assert!(
                mutated.validate_retained_structure().is_err(),
                "v6 accepted mutated {field}"
            );
            assert!(
                mutated.identity_sha256().is_err(),
                "v6 hashed mutated {field}"
            );
            assert!(
                mutated.validate_current_dependency().is_err(),
                "v6 admitted mutated {field}"
            );
            encoded[field] = original;
        }
    }

    #[test]
    fn retained_v7_oracle_contract_is_exact_but_never_current() {
        let retained = OracleVersionContract {
            schema_version: 7,
            tantivy_version: ORACLE_V7_TANTIVY_VERSION.to_owned(),
            tantivy_source: ORACLE_V7_TANTIVY_SOURCE.to_owned(),
            tantivy_checksum_sha256: ORACLE_V7_TANTIVY_CHECKSUM_SHA256.to_owned(),
            lexical_package: ORACLE_V7_LEXICAL_PACKAGE.to_owned(),
            lexical_package_version: ORACLE_V7_LEXICAL_PACKAGE_VERSION.to_owned(),
            lexical_contract_audit_revision: ORACLE_V7_LEXICAL_CONTRACT_AUDIT_REVISION.to_owned(),
        };
        retained
            .validate_retained_structure()
            .expect("the committed v7 dependency record remains decode-only evidence");
        assert!(retained.validate_stored_structure().is_err());
        assert!(retained.validate_current_dependency().is_err());
        assert_eq!(
            retained.identity_sha256().expect("retained v7 identity"),
            "5552a08677e98b660e25bc9b178e48bb96a49fd2b13989d39af57230f96c0558",
            "retained evidence must keep the exact v7 identity",
        );

        let encoded = serde_json::to_value(&retained).expect("serialize retained v7");
        for (field, value) in [
            ("schema_version", serde_json::json!(8)),
            (
                "tantivy_version",
                serde_json::json!(CURRENT_ORACLE_TANTIVY_VERSION),
            ),
            (
                "tantivy_source",
                serde_json::json!("git+https://example.invalid/tantivy"),
            ...
            (
                "tantivy_checksum_sha256",
                serde_json::json!(ORACLE_V8_TANTIVY_CHECKSUM_SHA256),
            ),
            ("lexical_package", serde_json::json!("lookalike-lexical")),
            ("lexical_package_version", serde_json::json!("0.2.999")),
            (
                "lexical_contract_audit_revision",
            ...
            mutated[field] = value;
            let mutated: OracleVersionContract =
                serde_json::from_value(mutated).expect("well-typed mutation");
            assert!(
                mutated.validate_retained_structure().is_err(),
                "v7 accepted mutated {field}"
            );
            assert!(
                mutated.identity_sha256().is_err(),
                "v7 hashed mutated {field}"
            );
        }

        let mut relabeled = retained;
        relabeled.tantivy_version = CURRENT_ORACLE_TANTIVY_VERSION.to_owned();
        relabeled.tantivy_checksum_sha256 = ORACLE_V8_TANTIVY_CHECKSUM_SHA256.to_owned();
        assert!(relabeled.validate_retained_structure().is_err());
    }

    #[test]
    fn retained_v8_oracle_contract_is_exact_but_never_current() {
        let contract = OracleVersionContract {
            schema_version: 8,
            tantivy_version: ORACLE_V8_TANTIVY_VERSION.to_owned(),
            ...
            lexical_package_version: ORACLE_V8_LEXICAL_PACKAGE_VERSION.to_owned(),
            lexical_contract_audit_revision: ORACLE_V8_LEXICAL_CONTRACT_AUDIT_REVISION.to_owned(),
        };
        contract.validate_retained_structure().expect("retained v8");
        ...
        assert_eq!(
            ...
            "6b29d3de2f15784574c605ed63ad35b7f0691f99f961981a3c03d6a69dae152a",
            "v8 must retain the identity of the original 0.2.5 wrapper record",
        );
    ...
    #[test]
    fn retained_v9_oracle_contract_is_exact_but_never_current() {
        let contract = OracleVersionContract {
            schema_version: 9,
            tantivy_version: ORACLE_V8_TANTIVY_VERSION.to_owned(),
            tantivy_source: ORACLE_V8_TANTIVY_SOURCE.to_owned(),
            tantivy_checksum_sha256: ORACLE_V8_TANTIVY_CHECKSUM_SHA256.to_owned(),
            lexical_package: ORACLE_V8_LEXICAL_PACKAGE.to_owned(),
            lexical_package_version: ORACLE_V9_LEXICAL_PACKAGE_VERSION.to_owned(),
            lexical_contract_audit_revision: ORACLE_V9_LEXICAL_CONTRACT_AUDIT_REVISION.to_owned(),
        };
        contract.validate_retained_structure().expect("retained v9");
        assert!(contract.validate_stored_structure().is_err());
        assert!(contract.validate_current_dependency().is_err());
        assert_eq!(
            contract.identity_sha256().expect("retained identity"),
            "eb81f46da35c3250da4273c25b68834261311d48b453a8b481410b0e2004104f",
            "v9 retains the original 0.3.0 wrapper identity",
        );
        let encoded = serde_json::to_value(&contract).expect("serialize v9");
        for (field, value) in [
            ("schema_version", serde_json::json!(10)),
            ("tantivy_version", serde_json::json!("0.26.1")),
            (
                "tantivy_source",
                serde_json::json!("git+https://example.invalid/tantivy"),
            ),
            ...
            (
                "lexical_package_version",
                serde_json::json!(ORACLE_V10_LEXICAL_PACKAGE_VERSION),
            ),
            (
                "lexical_contract_audit_revision",
                serde_json::json!(ORACLE_V10_LEXICAL_CONTRACT_AUDIT_REVISION),
            ),
        ] {
            let mut mutated = encoded.clone();
            mutated[field] = value;
            let mutated: OracleVersionContract = serde_json::from_value(mutated).unwrap();
            assert!(
                mutated.validate_retained_structure().is_err(),
                "v9 accepted {field}"
            );
            assert!(mutated.identity_sha256().is_err(), "v9 hashed {field}");
            assert!(
                mutated.validate_current_dependency().is_err(),
                "v9 admitted {field}"
            );
