...
//! first real observer) and run standalone on the per-call path today.
...
//
// These four items are signature-identical to the seam the H1 continuous
// window candidate (bd-qg1-immutable-continuous-timing-vdljy) defines in
// `continuous.rs`. H1 is not yet landed; when it lands, its rebase should
// delete its local definitions and import these, so exactly one definition
// exists. The only H2 additions are additive: serde derives and `label()` on
// `LifecyclePhase` (H1 call sites are unaffected).
...
pub enum CensusCaveat {
    ...
    /// for less than one tick (including any new thread alive under ~1ms)
    /// can read zero CPU while having actually run.
