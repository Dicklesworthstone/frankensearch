...
/// applied conservatively (an all-tombstoned v1 slab fails closed). A
/// refused non-empty cycle aborts before draining, leaving queued jobs and
/// retry counts untouched. Fully admitted v2 replacements can be staged and
...
mod tests {
    ...
    /// Pairwise certified-conformance shape (same space, byte-identical
    /// pinned golden-vector certificate): the predicate the frozen r3 chain
    /// exposed as `EmbeddingIdentityBundleV1::is_conformance_compatible_with`,
    /// inlined here because the trunk's generation API superseded the
    /// pairwise method with the witness/certificate flow.
    fn conformance_compatible(
        a: &EmbeddingIdentityBundleV1,
        b: &EmbeddingIdentityBundleV1,
    ) -> bool {
        a.validate().is_ok()
            && b.validate().is_ok()
            && a.space.fingerprint() == b.space.fingerprint()
            && a.producer.golden_vectors == b.producer.golden_vectors
    }
    ...
    fn certified_sibling_producer_refused_attestation_unavailable() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            assert!(
                conformance_compatible(&sibling, &base) && conformance_compatible(&base, &sibling),
    ...
    fn two_faced_embedder_refused_at_bound_record_seam() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            assert!(conformance_compatible(&bound_side, &public));
    ...
    /// cannot match the target's main slab. Built generation-RELATIVE (the
    /// authority-model `publish_tier` bumps the canonical generation on
    /// every publish, so absolute values are not stable): the donor main
    /// carries `next(target)`, so its WAL carries `next(next(target))`,
    /// which can never equal the `next(target)` the target expects.
    ///
    ...
    fn transplant_stale_wal(label: &str, target_main: &Path, dimension: usize) {
        ...
        let target_gen =
            VectorIndex::peek_compaction_gen(target_main).expect("peek target generation");
        let donor_dir = temp_index_dir(&format!("{label}-wal-donor"));
        let donor_main = donor_dir.join("donor.idx");
        let mut writer = VectorIndex::create(&donor_main, "stub-fast", dimension)
            .expect("create donor")
            .with_generation(frankensearch_index::next_generation(target_gen));
        writer
        ...
        writer.finish().expect("finish donor");
        {
            ...
            donor
                ...
                .expect("append the donor WAL resident");
        }
    ...
    fn legacy_crash_state_generation_collision_refuses_pre_drain_without_embedding() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            // Simulate the pre-authority crash point: a replacement main that
            // did NOT bump the generation (the old in-place republish shape)
            // is renamed over the canonical name; the WAL removal never ran.
            // Generation-RELATIVE: the replacement carries the OLD main's
            // generation, so the leftover `next(old)` WAL collides exactly.
            let old_gen =
                VectorIndex::peek_compaction_gen(&fast_path).expect("peek old generation");
            let scratch = dir.join("replacement.scratch");
            {
                let mut writer = VectorIndex::create(&scratch, "stub-fast", 256)
