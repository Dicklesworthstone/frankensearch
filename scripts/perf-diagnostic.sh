#!/usr/bin/env bash
# perf-diagnostic.sh — DIAGNOSTIC-ONLY machine-class provenance profiler.
#
# ##########################################################################
# # THIS TOOL PRODUCES NO PROMOTABLE EVIDENCE.                             #
# #                                                                        #
# # Nothing it writes is a certified receipt. Its output must never be     #
# # promoted, ratcheted, or cited as gate evidence. Certified measurement  #
# # runs through scripts/perf-runner.sh, which drives the typed Rust       #
# # producer (quill-perf-finalize) that owns the exclusive lease, the      #
# # start/end host probes, the artifact-manifest binding, the receipt      #
# # sealing, and self-admission.                                          #
# #                                                                        #
# # This script IS the "diagnostic-only profiler" that the promotion       #
# # producer's own header defers to:                                       #
# #   "Diagnostic Apple profiling happens outside this promotion producer."#
# ##########################################################################
#
# What this is for — the capabilities the certified launcher deliberately
# does not have, and deliberately should not have:
#   * running an ARBITRARY benchmark command under recorded provenance
#     (the certified runner resolves its benchmark from a clean source
#     snapshot, by construction, and accepts no command)
#   * --calibrate-aa A/A calibration runs
#   * profiling a host BEFORE it is admitted to the machine-class registry.
#     This script's fingerprint/ output is the raw material a host is
#     registered FROM: docs/evidence/e8h/fingerprints/*/ holds exactly these
#     artifacts, and machine_class_registry.rs include_bytes!()s them.
#   * Apple M4/M5 profiling, where certified promotion currently fails
#     closed pending an executing-image attestation mechanism.
#
# --- machine-class taxonomy: mapping onto the certified runner ------------
#
# This script's --class values map onto the certified runner's
# --hardware-class / --execution-profile pair as follows. Every class value
# is reachable there; none was dropped in the 2026-08-02 harmonization.
#
#   --class (here)   --hardware-class   --execution-profile   promotion status
#   --------------   ----------------   -------------------   ----------------
#   x86-vps-ovh      x86-vps-ovh        x86-diagnostic        diagnostic-only
#   trj-zen-128c     trj-zen3-5995wx    smt2-128              available
#   trj-zen3-<N>c    trj-zen3-5995wx    physical-64           available
#   m4-macos         m4-macos           scheduler-10          fails closed
#   m5-macos         m5-macos           scheduler-14          unavailable
#
# `trj-zen-128c` and the width-encoded `trj-zen3-<N>c` forms are LEGACY
# EXECUTION labels, not hardware classes. parse_hardware_class_id() in
# machine_class_registry.rs rejects both with ObsoleteClassId, and the table
# above is that rejection's migration path. The two labels marked
# diagnostic-only / fails-closed are precisely the cells this script serves.
#
# Host probing is owned by the certified producer, which carries the macOS
# probes this script pioneered: pmset -g therm (local_perf_runner.rs
# macos_probe) and sysctl hw.pagesize, alongside the Linux getconf PAGESIZE
# and k10temp/zenpower thermal-sensor probes. This script keeps its own
# copies because it must run on hosts the producer will not accept yet.
#
# The emitted provenance schema string stays "frankensearch.perf-runner.v1":
# it names the ARTIFACT format, and the already-registered fingerprints under
# docs/evidence/e8h/fingerprints/ carry that exact string and are hashed as
# bytes. Renaming it here would silently fork the format.
#
# --- usage ----------------------------------------------------------------
#
#   scripts/perf-diagnostic.sh --class <id> [options] [-- <command...>]
#
...
#                      (exports QUILL_PERF_CALIBRATE_AA=1)
...
#   --out <dir>        output root (default: ~/.frankensearch-perf-diagnostics,
#                      or $PERF_DIAGNOSTIC_OUT). Deliberately NOT the certified
#                      runner's root, so diagnostic directories can never be
#                      mistaken for sealed receipts.
#   -- <command...>    command to run; default:
#                      cargo bench -p frankensearch-quill-gauntlet
#                        --bench perf_matrix --features perf-harness
#                        --profile release-perf
#
# Captures the per-class provenance that docs/contracts/quill-hyperopt-campaign.md
# § 2 makes mandatory: CPU/NUMA topology, governor + SMT state (Linux),
# thermal pressure + page size (macOS), git revision + dirty-state hash,
# toolchain versions.
#
...
FOREGROUND=0
ALLOW_RCH=0
OUT_ROOT="${PERF_DIAGNOSTIC_OUT:-$HOME/.frankensearch-perf-diagnostics}"

# Print the header block (line 2 through the last leading-# line) as help.
usage() { sed -n '2,${/^#/!q;s/^# \{0,1\}//;s/^#$//;p;}' "$0"; }
...
echo "DIAGNOSTIC ONLY — this run produces no promotable evidence."
