...
distinct queries (2026-09-23, Threadripper PRO 5975WX):
...
query as plain text, which is how related wording still surfaces. An
exclusion (`-fusion`, `-"rank fusion"` or `NOT "rank fusion"`) applies to
every tier: the vector tiers read the query without it and skip each file
the BM25 tier matches for it. The BM25 tier also understands:
...
| `+rrf -fusion` | `rrf` required in the word match; files containing `fusion` excluded from all results |
...
|---|---|---|---|
| SciFact (5,183 / 300) | 0.653 | 0.590 | 0.689 |
| NFCorpus (3,633 / 323) | 0.301 | 0.291 | 0.332 |
| ArguAna (8,674 / 1,406) | 0.315 | 0.336 | 0.363 |
...
results and `fast_only` search rank no better than plain lexical search.

The cross-encoder (`--rerank`, ms-marco-MiniLM-L-6-v2) applies to these
abstract-length documents only with a raised `rerank_timeout_ms`. With it, at
limit 10, reranking changed nDCG@10 by +0.021 on SciFact (not significant),
+0.016 on NFCorpus (significant) and −0.013 on ArguAna (significantly worse),
so reranking stays opt-in and its 300 ms default deadline is unchanged.
...
or both before sharing the index with a `TwoTierSearcher`:

```rust,ignore
use frankensearch_index::native_hnsw::HnswParams;

// `index` is an already admitted FSVI v2 TwoTierIndex.
index.enable_native_fast_hnsw(HnswParams::default(), 42)?;
index.enable_native_quality_hnsw(HnswParams::default(), 43)?;
```

Each graph shares its retained vector owner. The progressive fast phase retrieves
native ANN candidates and exactly rescores them before fusion. Quality retrieval
searches independently of the fast candidate pool and exactly rescores its own
candidates. Successful refinement retains both tiers' coverage, including the
candidate counts after exclusions; failed refinement preserves the fast receipt.

Query admission checks the identity that actually accompanies the provider's
bound embedding, including through cached providers. Bound requests bypass the
raw-vector cache because that cache does not retain response identities.
An explicit `SearchParams` override uses an exact fast-tier scan after identity
admission, bypassing native ANN and MRL candidate selection. Native retrieval is
available through `ActivatedTierSearch` and `TwoTierSearcher`; the raw-vector
`TwoTierIndex::search_fast` API keeps its existing path.
...
query daemon). Both are regenerated by `QUALITY_GATE_STAGES=perf
scripts/quality-gate.sh`. Rows marked *target* have no committed measurement
...
| `fsfs` query over 4,096 characters, or holding a control character | cut to its first 4,096 characters; control characters read as spaces | every phase, on the cleaned query (payload `query` shows it) |
...
| On-disk vector format | [`crates/frankensearch-index/src/quantization.rs`](crates/frankensearch-index/src/quantization.rs) | FSVI quantization; mmap via [`mapped_file.rs`](crates/frankensearch-index/src/mapped_file.rs) |
