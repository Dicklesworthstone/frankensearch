...
|---------|------|------|---------|
| frankensearch-quill 0.3.4 | Hotfix crate on frankensearch-quill 0.3.3 (branch `hotfix/quill-0.3.4-union-seek`, tag `frankensearch-quill-v0.3.4`) | 2026-09-25 | A disjunction under a conjunction, such as `(a OR b) AND c` (and every legacy-grammar `a OR b c`), no longer loses matches on concat-merged segments. `BufferedUnionScorer::seek_danger` answered a target at or before its current document from its drained children instead of its buffer, so the intersection skipped buffered matches. A layout-sweep regression test (84 layouts, merged and with tombstones) had 2420 wrong answers before the fix and none after. frankensearch 0.6.1 already accepts quill `^0.3.1` |
| [frankensearch-quill 0.3.3](https://crates.io/api/v1/crates/frankensearch-quill/0.3.3) | Hotfix crate on frankensearch-quill 0.3.2 (branch `hotfix/quill-0.3.3-cass-grammar`, tag `frankensearch-quill-v0.3.3`) | 2026-09-24 | CASS queries use standard Boolean precedence (NOT, then AND, explicit or implied, then OR) and parentheses group; `(` groups only at the start of a word, so `foo(bar)` stays one term, and unbalanced parentheses are recovered with diagnostics (cass bead 2l1b0.52). The lexical cass_compat oracle still implements the legacy grammar, so the Quill/oracle differential covers only queries on which both grammars agree; aligning the oracle is follow-up work for main. frankensearch 0.6.1 already accepts quill `^0.3.1` |
| [frankensearch-quill 0.3.2](https://crates.io/api/v1/crates/frankensearch-quill/0.3.2) | Hotfix crate on frankensearch 0.6.1 (branch `hotfix/quill-0.3.2-gh499-v2`, tag `frankensearch-quill-v0.3.2`) | 2026-09-24 | Numeric range/set leaves score on live documents, so a date-filtered Boolean over a sealed segment with tombstones no longer fails with "Boolean children belong to different segment domains" (#49, cass GH #499). Carries 86337594 with its test follow-ups e2eb8d74 and 3ab53b95, plus a cass-shaped regression test that fails on 0.3.1. frankensearch 0.6.1 already accepts quill `^0.3.1` |
