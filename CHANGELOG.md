...
- **Watch mode on an existing index no longer dies at startup** (bd-ql03m). The live pipeline opened its writer within milliseconds of the one-shot pass releasing it and could hit a transient `fsvi.map_lock` refusal; it now uses the same bounded retry as every other in-place mutation.
