...
- **`frankensearch-index` compiles for x86_64 macOS again.** The generation-root host checks (a1990de7) gated their macOS platform helpers on `aarch64` while their callers were unconditional, so the `x86_64-apple-darwin` lite asset could not be built; the gates now cover all of macOS (the arm64 and x86_64 branches use the same calls; the arm64-host-only test modules keep their gate).
