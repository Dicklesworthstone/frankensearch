...
|---------|------|------|---------|
| frankensearch-quill 0.3.2 | Hotfix crate on frankensearch 0.6.1 (branch `hotfix/quill-0.3.2-gh499-v2`; unpublished at time of writing) | 2026-09-24 | Numeric range/set leaves score on live documents, so a date-filtered Boolean over a sealed segment with tombstones no longer fails with "Boolean children belong to different segment domains" (#49, cass GH #499). Carries 86337594 with its test follow-ups e2eb8d74 and 3ab53b95, plus a cass-shaped regression test that fails on 0.3.1. frankensearch 0.6.1 already accepts quill `^0.3.1` |
