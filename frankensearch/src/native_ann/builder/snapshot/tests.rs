...
async fn build(cx: &Cx, path: &Path, ann: bool, quality: bool) -> NativeBuiltIndex {
    let mut builder = NativeIndexBuilder::new(path, generation(), provider("fast", 2))
        .unwrap()
        .add_documents(documents());
    if ann {
        builder = builder.with_fast_storage(
    ...
    if quality {
        builder = builder
            .with_quality_embedder(provider("quality", 3))
            .unwrap();
    }
...
fn sealed_exact_and_ann_builds_reopen_all_tiers_without_reembedding() {
    asupersync::test_utils::run_test_with_cx(|cx| async move {
        for ann in [false, true] {
            ...
            let quality_hits = built
                .quality()
                .unwrap()
                .search(&cx, "vertical", 5)
                .await
                .unwrap();
            ...
            assert_eq!(
                reopened.quality().unwrap().index().owner_witness(),
                &quality_witness
            );
            assert_eq!(
                serde_json::to_value(reopened.documents()).unwrap(),
                original_documents
            );
            assert_eq!(
                reopened.fast().search(&cx, "vertical", 5).await.unwrap(),
                fast_hits
            );
            assert_eq!(
                reopened
...
fn reopened_sources_retain_titles_metadata_unicode_and_empty_text() {
    asupersync::test_utils::run_test_with_cx(|cx| async move {
        ...
        let built = NativeIndexBuilder::new(&path, generation(), provider("fast", 2))
            ...
            .add_document(IndexableDocument::new("empty", ""))
            .build(&cx)
            .await
            .unwrap();
        let receipt = built.seal_for_reopen(&cx).unwrap();
        ...
        assert_eq!(
            serde_json::to_value(reopened.document("文書").unwrap()).unwrap(),
    ...
    asupersync::test_utils::run_test_with_cx(|cx| async move {
        ...
        for tier in [reopened.fast(), reopened.quality().unwrap()] {
            ...
            let physical_ids: Vec<_> = (0..owner.record_count())
                ...
                .collect();
            assert_ne!(
                physical_ids, source_ids,
                "fixture must expose physical hash ordering"
            );
            ...
            assert_eq!(
                tier.search(&cx, "vertical", 1).await.unwrap()[0].doc_id,
                "b"
            );
        }
    });
...
fn receipt_reopens_after_directory_relocation_and_reader_owners_are_independent() {
    asupersync::test_utils::run_test_with_cx(|cx| async move {
        ...
        assert_eq!(
            one.fast().index().owner_witness(),
            two.fast().index().owner_witness()
        );
        ...
        drop(one);
        assert_eq!(
            two.fast().search(&cx, "vertical", 1).await.unwrap()[0].doc_id,
            "b"
        );
...
fn empty_selected_cohort_reopens_with_a_real_nonempty_source_receipt() {
    asupersync::test_utils::run_test_with_cx(|cx| async move {
        ...
        let receipt = built.seal_for_reopen(&cx).unwrap();
        assert_eq!(
            std::fs::read(path.join(SOURCE_FILE)).unwrap(),
            SOURCE_HEADER
        );
        let reopened = NativeBuiltIndex::open_selected_with_limits(
            &cx,
            &path,
            &receipt,
            provider("fast", 2),
...
fn no_descriptor_means_no_implicit_rebuild_or_directory_discovery() {
    asupersync::test_utils::run_test_with_cx(|cx| async move {
        ...
        let receipt = GenerationComponentReceiptV1 {
            byte_len: 100,
            sha256: [1; 32],
        };
...
fn substituted_producer_fails_before_reading_source_or_running_inference() {
    asupersync::test_utils::run_test_with_cx(|cx| async move {
        ...
        let error = NativeBuiltIndex::open_selected(&cx, &path, &receipt, foreign.clone(), None)
            .err()
            .unwrap();
        assert!(
            matches!(error, SearchError::InvalidConfig { ref field, .. } if field == "native_ann.builder.producer")
        );
...
fn equal_dimension_foreign_quality_does_not_drop_to_fast_only() {
    asupersync::test_utils::run_test_with_cx(|cx| async move {
        ...
        let receipt = built.seal_for_reopen(&cx).unwrap();
        assert!(
            NativeBuiltIndex::open_selected(
                &cx,
                &path,
                &receipt,
                provider("fast", 2),
                Some(provider("foreign-quality", 3))
            )
            .is_err()
        );
    });
...
fn original_external_receipt_rejects_an_otherwise_valid_descriptor_substitution() {
    asupersync::test_utils::run_test_with_cx(|cx| async move {
        ...
        let receipt = built.seal_for_reopen(&cx).unwrap();
        let _forged_pin =
            change_manifest(&path, |value| value["generation"]["sequence"] = 10.into());
        assert!(open(&cx, &path, &receipt, false).is_err());
    });
}
...
fn valid_vector_from_same_model_and_generation_cannot_replace_selected_bytes() {
    asupersync::test_utils::run_test_with_cx(|cx| async move {
        ...
        let changed: Vec<_> = documents()
            .into_iter()
            .map(|mut doc| {
                doc.content = "vertical".to_owned();
                doc
            })
            .collect();
        let second =
            NativeIndexBuilder::new(dir.path().join("other"), generation(), provider("fast", 2))
                .unwrap()
                .add_documents(changed)
                .build(&cx)
                .await
                .unwrap();
        assert_eq!(
            first.fast().index().owner_witness().generation,
            second.fast().index().owner_witness().generation
        );
...
fn another_valid_native_graph_over_the_same_owner_is_not_the_selected_graph() {
    asupersync::test_utils::run_test_with_cx(|cx| async move {
        ...
        let replacement =
            NativeAnnIndex::build(&cx, Arc::clone(&built.fast.index.owner), params, 7).unwrap();
        replacement
            .save(&cx, built.fast().graph_path().unwrap())
            .unwrap();
        assert_eq!(
            std::fs::metadata(built.fast().graph_path().unwrap())
                .unwrap()
                .len(),
        ...
        assert!(
            NativeAnnIndex::load(
                &cx,
                Arc::clone(&built.fast.index.owner),
                built.fast().graph_path().unwrap()
            )
            .is_ok()
        );
        assert!(open(&cx, &path, &receipt, false).is_err());
...
fn selected_exact_mode_ignores_unselected_sidecars_instead_of_autodetecting_them() {
    asupersync::test_utils::run_test_with_cx(|cx| async move {
        ...
        assert!(matches!(
            reopened.fast().index().retrieval_mode(),
            crate::native_ann::NativeRetrievalMode::Exact { .. }
        ));
    });
}
...
fn resource_limits_refuse_before_returning_any_selected_handle() {
    asupersync::test_utils::run_test_with_cx(|cx| async move {
        ...
        for limits in [
            NativeReopenLimits {
                max_source_bytes: SOURCE_HEADER.len() as u64,
                ..NativeReopenLimits::default()
            },
            NativeReopenLimits {
            ...
            assert!(
                NativeBuiltIndex::open_selected_with_limits(
                    &cx,
                    &path,
                    &receipt,
                    provider("fast", 2),
    ...
    asupersync::test_utils::run_test_with_cx(|cx| async move {
        for mutation in [
            ...
            match mutation {
                ...
                "extra-doc" => {
                    let extra = lines.last().unwrap().clone();
                    lines.push(extra);
                }
                _ => {}
            }
            let mut source = lines.join("\n");
            if mutation != "truncated-line" {
                source.push('\n');
            }
            ...
            let receipt = change_manifest(&path, |value| {
                value["source"] = serde_json::to_value(artifact).unwrap();
            });
...
fn unknown_descriptor_schema_and_fields_are_not_forward_guessed() {
    asupersync::test_utils::run_test_with_cx(|cx| async move {
        for unknown_field in [false, true] {
            ...
            let receipt = change_manifest(&path, |value| {
                if unknown_field {
                    value["fallback_to_latest"] = true.into();
                } else {
                    value["schema"] = "future-version".into();
                }
            });
            assert!(open(&cx, &path, &receipt, false).is_err());
        }
...
fn existing_complete_or_partial_seals_are_never_overwritten() {
    asupersync::test_utils::run_test_with_cx(|cx| async move {
        ...
        assert_eq!(
            std::fs::read(partial.directory().join(SOURCE_FILE)).unwrap(),
            b"partial source"
        );
