...
mod tests {
    ...
    use frankensearch_core::generation::{
        ArtifactGenerationIdentityV1, EmbeddingArtifactIdentityV1,
        EmbeddingIdentityBundleV1, EmbeddingSpaceKindV1,
    };
    use frankensearch_core::traits::{IdentityBoundEmbedding, ModelCategory, RerankDocument, RerankScore};
    ...
    impl Provider {
        fn new(name: &str, dimension: u32) -> Self {
            ...
            Self { identity, calls: AtomicUsize::new(0), drops: AtomicUsize::new(0), mode: AtomicUsize::new(0) }
        }
        ...
        fn embed<'a>(&'a self, _cx: &'a Cx, text: &'a str) -> SearchFuture<'a, Vec<f32>> {
            Box::pin(async move {
                ...
                match self.mode.load(Ordering::SeqCst) {
                    ...
                    2 => Err(crate::native_ann::invalid("test.provider", "failed", "controlled failure")),
                    3 => Err(SearchError::Cancelled { phase: "test.provider".to_owned(), reason: "cancelled".to_owned() }),
                    _ => Ok(self.values(text)),
                }
            })
        }
        fn embed_batch_bound<'a>(&'a self, _cx: &'a Cx, texts: &'a [&'a str]) -> SearchFuture<'a, Vec<IdentityBoundEmbedding>> {
            Box::pin(async move {
    ...
    #[derive(Default)]
    struct RerankProbe { calls: AtomicUsize, drops: AtomicUsize, pending: bool }
    impl Reranker for RerankProbe {
        fn rerank<'a>(&'a self, _cx: &'a Cx, _query: &'a str, documents: &'a [RerankDocument]) -> SearchFuture<'a, Vec<RerankScore>> {
            Box::pin(async move {
                ...
                Ok(documents.iter().enumerate().map(|(rank, document)| RerankScore {
                    doc_id: document.doc_id.clone(),
                    score: if document.doc_id == "b" { 1.0 } else { 0.0 },
                    original_rank: rank, raw_logit: None,
                }).collect())
            })
        }
        fn id(&self) -> &'static str { "deadline-reranker" }
        fn model_name(&self) -> &str { self.id() }
    }
    ...
    async fn fixture(cx: &Cx, with_quality: bool) -> (tempfile::TempDir, Arc<Provider>, Arc<Provider>, NativeBuiltHybridIndex) {
        let directory = tempfile::tempdir().unwrap();
        let fast = Arc::new(Provider::new("fast", 2));
        let quality = Arc::new(Provider::new("quality", 3));
        let mut builder = NativeIndexBuilder::new(directory.path().join("old"), generation(1), fast.clone()).unwrap()
        ...
        if with_quality { builder = builder.with_quality_embedder(quality.clone()).unwrap(); }
    ...
    fn clock(cx: &Cx) -> (Arc<VirtualClock>, NativeSearchDeadline) {
        ...
        let deadline = NativeSearchDeadline::with_timer(cx, timer, Duration::from_millis(10)).unwrap();
        (clock, deadline)
    }
    ...
    fn assert_same_page(expected: &[ScoredResult], actual: &[ScoredResult]) {
        ...
        for (before, after) in expected.iter().zip(actual) {
            ...
            assert_eq!(before.fast_score.map(f32::to_bits), after.fast_score.map(f32::to_bits));
            assert_eq!(before.quality_score.map(f32::to_bits), after.quality_score.map(f32::to_bits));
            assert_eq!(before.lexical_score.map(f32::to_bits), after.lexical_score.map(f32::to_bits));
            assert_eq!(before.rerank_score.map(f32::to_bits), after.rerank_score.map(f32::to_bits));
    ...
    fn quality_expiry_preserves_initial_and_never_starts_the_queued_reranker() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            let mut stream = index.progressive_with_reranker_before(&cx, "vertical", 1, &reranker, 3, deadline).unwrap();
            ...
            let Some(NativeSearchPhase::RefinementFailed { initial_results, error }) = pending.await.unwrap() else { panic!("timeout phase") };
            assert!(matches!(error, SearchError::SearchTimeout { budget_ms: 10, elapsed_ms: 10 }));
    ...
    fn rerank_expiry_carries_the_exact_refined_page() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            clock.advance(10_000_000);
            let _ = timer.process_timers();
            let Some(NativeSearchPhase::RerankFailed { previous_results, error }) = pending.await.unwrap() else { panic!("rerank timeout") };
    ...
    fn consumer_delay_uses_the_same_budget_and_expired_phases_do_not_start() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            for after_initial in [false, true] {
                let (_directory, fast, quality, index) = fixture(&cx, true).await;
                let (clock, deadline) = clock(&cx);
                let mut stream = index.progressive_before(&cx, "vertical", 1, deadline).unwrap();
                if after_initial { assert!(stream.next_phase().await.unwrap().is_some()); }
                clock.advance(11_000_000);
                let phase = stream.next_phase().await;
                if after_initial {
                    assert!(matches!(phase, Ok(Some(NativeSearchPhase::RefinementFailed { error: SearchError::SearchTimeout { .. }, .. }))));
                } else {
                    assert!(matches!(phase, Err(SearchError::SearchTimeout { .. })));
                }
                assert_eq!(fast.calls.load(Ordering::SeqCst), usize::from(after_initial));
                assert_eq!(quality.calls.load(Ordering::SeqCst), 0);
    ...
    fn cancellation_never_becomes_timeout_degradation() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            for provider_cancel in [false, true] {
                ...
                let mut stream = index.progressive_before(&cx, "vertical", 1, deadline).unwrap();
                assert!(stream.next_phase().await.unwrap().is_some());
                quality.mode.store(if provider_cancel { 3 } else { 1 }, Ordering::SeqCst);
                let mut pending = Box::pin(stream.next_phase());
                if !provider_cancel {
                    assert!(pending.as_mut().poll(&mut Context::from_waker(Waker::noop())).is_pending());
        ...
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            for with_quality in [false, true] {
                ...
                if with_quality {
                    ...
                    assert!(matches!(stream.next_phase().await.unwrap(), Some(NativeSearchPhase::RefinementFailed { .. })));
                }
                ...
                assert_eq!(fast.calls.load(Ordering::SeqCst), 1);
                assert_eq!(quality.calls.load(Ordering::SeqCst), usize::from(with_quality));
                assert_eq!(reranker.calls.load(Ordering::SeqCst), 1);
                assert!(stream.is_finished());
            }
        });
    ...
    fn live_replacement_does_not_change_an_expiring_queries_page_or_owners() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            let (clock, deadline) = clock(&cx);
            let mut stream = pin.index().progressive_before(&cx, "vertical", 1, deadline).unwrap();
            let Some(NativeSearchPhase::Initial { results, .. }) = stream.next_phase().await.unwrap() else { panic!("initial") };
            quality.mode.store(1, Ordering::SeqCst);
            let mut pending = Box::pin(stream.next_phase());
            assert!(pending.as_mut().poll(&mut Context::from_waker(Waker::noop())).is_pending());
            let candidate = pin.begin_update(&cx, directory.path().join("next"), generation(2)).unwrap()
                .upsert_document(IndexableDocument::new("b", "horizontal").with_metadata("version", "new"))
                .build(&cx).await.unwrap();
            live.install(&cx, &candidate).await.unwrap();
            clock.advance(10_000_000);
            let Some(NativeSearchPhase::RefinementFailed { initial_results, .. }) = pending.await.unwrap() else { panic!("timeout") };
            assert_same_page(&results, &initial_results);
            let row = usize::try_from(initial_results[0].index.unwrap()).unwrap();
            assert_eq!(pin.index().vectors().fast().index().owner.doc_id_at(row).unwrap(), "b");
            assert_eq!(pin.index().vectors().document("b").unwrap().content, "vertical");
            ...
            assert_eq!(current.snapshot.index().vectors().document("b").unwrap().metadata["version"], "new");
            assert!(stream.is_finished());
        });
    }
    ...
    fn bounded_and_unbounded_complete_phases_agree_without_expiry() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            for with_quality in [false, true] {
                ...
                for k in [0, 1] {
                    let (_, deadline) = clock(&cx);
                    let mut bounded = index.progressive_before(&cx, "vertical", k, deadline).unwrap();
                    let mut reference = index.progressive(&cx, "vertical", k).unwrap();
                    loop {
                        match (reference.next_phase().await.unwrap(), bounded.next_phase().await.unwrap()) {
                            (Some(NativeSearchPhase::Initial { results: a, candidates: ca }),
                             Some(NativeSearchPhase::Initial { results: b, candidates: cb }))
                            | (Some(NativeSearchPhase::Refined { results: a, candidates: ca }),
                               Some(NativeSearchPhase::Refined { results: b, candidates: cb })) => {
