...
mod selected_restart {
    ...
    async fn reopen(cx: &Cx, path: &Path, receipt: &GenerationComponentReceiptV1) -> SearchResult<NativeBuiltHybridIndex> {
        NativeBuiltHybridIndex::open_selected(
            cx, path, receipt, Arc::new(Provider::new("fast", 2, Reply::Short)), None,
        ).await
    }
    ...
    fn selected_hybrid_restores_quill_sources_all_vector_tiers_and_lazy_phases() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            for ann in [false, true] {
                ...
                let built = NativeIndexBuilder::new(&path, generation(), fast).unwrap()
                    .add_documents(documents).with_fast_storage(NativeBuildPrecision::F16, retrieval)
                    .with_quality_embedder(quality).unwrap().build_hybrid(&cx).await.unwrap();
                ...
                assert_eq!(fast.queries.load(Ordering::SeqCst), 0);
                assert_eq!(quality.queries.load(Ordering::SeqCst), 0);
                assert_eq!(serde_json::to_value(restored.vectors().documents()).unwrap(), source);
                assert_eq!(restored.lexical().doc_count().unwrap(), 5);
                assert_eq!(restored.vectors().fast().index().retrieval_mode() == NativeRetrievalMode::Ann, ann);
                let reranker = RerankProbe::default();
                let mut stream = restored.progressive_with_reranker(&cx, "vertical", 5, &reranker, 5).unwrap();
                assert!(matches!(stream.next_phase().await.unwrap(), Some(NativeSearchPhase::Initial { .. })));
                assert_eq!(quality.queries.load(Ordering::SeqCst), 0);
                let Some(NativeSearchPhase::Refined { results, .. }) = stream.next_phase().await.unwrap() else { panic!("refined"); };
                ...
                assert!(results.iter().find(|hit| hit.doc_id == "b").unwrap().metadata.is_some());
                assert_eq!(reranker.calls.load(Ordering::SeqCst), 0);
                assert!(matches!(stream.next_phase().await.unwrap(), Some(NativeSearchPhase::Reranked { .. })));
                assert_eq!(reranker.documents.lock().unwrap().iter().find(|doc| doc.doc_id == "b").unwrap().text, "vertical");
                assert_eq!(fast.queries.load(Ordering::SeqCst), 1);
                assert_eq!(quality.queries.load(Ordering::SeqCst), 1);
            }
        });
    ...
    fn relocated_selected_hybrid_supports_independent_readers_without_writer_leases() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            let writer = QuillIndex::open(&cx, relocated.join("lexical"), QuillConfig::default())
                .await.expect("neither reopened hybrid retains a writer lease");
            LexicalWrite::index_document(&writer, &cx, &crate::IndexableDocument::new("late", "laterpublication")).await.unwrap();
            LexicalWrite::commit(&writer, &cx).await.unwrap();
            assert_eq!(first.lexical().doc_count().unwrap(), 5);
            drop(first);
            assert_eq!(second.search(&cx, "vertical", 1).await.unwrap()[0].doc_id, "b");
            assert!(second.lexical().search(&cx, "laterpublication", 10).await.unwrap().is_empty());
    ...
    fn same_cardinality_foreign_lexical_directory_cannot_replace_selected_bytes() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            for doc in &mut docs { doc.content = "foreignpublication".to_owned(); }
            let other_path = dir.path().join("other");
            let other = NativeIndexBuilder::new(&other_path, generation(), fast.clone()).unwrap()
                .add_documents(docs).build_hybrid(&cx).await.unwrap();
            assert_eq!(other.lexical().doc_count().unwrap(), built.lexical().doc_count().unwrap());
            ...
            assert_eq!(built.lexical().search(&cx, "vertical", 10).await.unwrap()[0].doc_id, "b");
        });
    }
    ...
    fn sealing_old_reader_does_not_bless_newer_lexical_publication() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            let fast = Arc::new(Provider::new("fast", 2, Reply::Correct));
            let built = builder(&path, &fast).build_hybrid(&cx).await.unwrap();
            let writer = QuillIndex::open(&cx, path.join("lexical"), QuillConfig::default()).await.unwrap();
            LexicalWrite::index_document(&writer, &cx, &crate::IndexableDocument::new("late", "laterpublication")).await.unwrap();
            LexicalWrite::commit(&writer, &cx).await.unwrap();
    ...
    fn missing_manifest_or_segment_never_becomes_vector_only_or_previous_lexical_success() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            for manifest in [true, false] {
                ...
                let built = builder(&path, &fast).build_hybrid(&cx).await.unwrap();
                let receipt = built.seal_for_reopen(&cx).unwrap();
                let encoded: serde_json::Value = serde_json::from_slice(&std::fs::read(path.join("native.hybrid.json")).unwrap()).unwrap();
                let name = if manifest { "MANIFEST" } else {
                    encoded["lexical"]["files"].as_array().unwrap().iter()
                        .find_map(|file| file["name"].as_str().filter(|name| name.ends_with(".fslx"))).unwrap()
                };
                std::fs::rename(path.join("lexical").join(name), dir.path().join("displaced-artifact")).unwrap();
                assert!(reopen(&cx, &path, &receipt).await.is_err());
    ...
    fn hybrid_input_limits_and_unsafe_descriptor_names_fail_closed() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            let receipt = built.seal_for_reopen(&cx).unwrap();
            for limits in [
                NativeHybridReopenLimits { max_lexical_files: 0, ..NativeHybridReopenLimits::default() },
                NativeHybridReopenLimits { max_lexical_file_bytes: 1, ..NativeHybridReopenLimits::default() },
                NativeHybridReopenLimits { max_lexical_bytes: 1, ..NativeHybridReopenLimits::default() },
            ] {
                assert!(NativeBuiltHybridIndex::open_selected_with_limits(&cx, &path, &receipt, fast.clone(), None, limits).await.is_err());
            }
            let file = path.join("native.hybrid.json");
            let original: serde_json::Value = serde_json::from_slice(&std::fs::read(&file).unwrap()).unwrap();
    ...
    fn changed_lock_is_not_part_of_read_only_hybrid_selection() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            let built = builder(&path, &fast).build_hybrid(&cx).await.unwrap();
            let receipt = built.seal_for_reopen(&cx).unwrap();
            std::fs::write(path.join("lexical").join("LOCK"), b"not a writer-admission record").unwrap();
            let restored = reopen(&cx, &path, &receipt).await.unwrap();
            assert_eq!(restored.search(&cx, "vertical", 1).await.unwrap()[0].doc_id, "b");
        });
    }
    ...
    fn partial_hybrid_seal_is_not_overwritten_or_admitted() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            assert!(built.seal_for_reopen(&cx).is_err());
            assert_eq!(std::fs::read(path.join("native.hybrid.json")).unwrap(), partial);
            assert!(!path.join("native.snapshot.json").exists());
            assert!(reopen(&cx, &path, &Artifact::from_bytes(partial).receipt()).await.is_err());
        });
    }
    ...
    fn cancelled_hybrid_seal_and_reopen_do_not_write_or_produce_partial_success() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            assert!(matches!(reopen(&cx, &path, &receipt).await, Err(SearchError::Cancelled { .. })));
            assert!(matches!(unsealed.seal_for_reopen(&cx), Err(SearchError::Cancelled { .. })));
