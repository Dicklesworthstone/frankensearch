...
impl NativeQuillSnapshot {
    ...
    pub fn admit(
        ...
        if fast.owner_witness() != expected_fast {
            return Err(invalid("quill.fast", "witness-mismatch", "native owner differs from the selected complete FSVI witness"));
        }
        expected_lexical.validate().map_err(|error| {
            invalid("quill.lexical", "invalid-receipt", &error.to_string())
        })?;
        if expected_lexical.role != GenerationComponentRole::Lexical {
            return Err(invalid("quill.lexical", "wrong-role", "the selected lexical receipt must have the lexical role"));
        }
        let snapshot = lexical.search_snapshot().map_err(SearchError::from)?;
        if snapshot.delta_count() != 0 {
            return Err(invalid("quill.lexical", "unsealed-delta", "a Keeper receipt cannot authenticate unsealed Delta epochs"));
        }
        let receipt = snapshot.keeper_snapshot()
            .exact_lexical_component_receipt(source_checkpoint)
            .map_err(QuillIndexError::from)
            .map_err(SearchError::from)?;
        if &receipt != expected_lexical {
            return Err(invalid("quill.lexical", "receipt-mismatch", "the actual lexical publication differs from its selected exact receipt"));
        }
    ...
    pub async fn search_text(
        ...
        self.lexical.check_current()?;
        self.fast.search_hybrid_text(cx, embedder, &self.lexical, text, k).await
    }
    ...
    pub fn search_progressive<'a>(
        ...
        self.lexical.check_current()?;
        self.fast.search_hybrid_progressive(cx, embedder, None, &self.lexical, text, k)
    }
}
...
fn join_docsets(vector: &ExactComponentReceiptV1, lexical: &ExactComponentReceiptV1) -> SearchResult<()> {
    ...
    {
        return Err(invalid("quill.docset", "component-mismatch", "native and lexical readers must authenticate the same canonical live-document set and source checkpoint"));
    }
    Ok(())
}
...
impl FencedQuillRead {
    fn check_current(&self) -> SearchResult<()> {
        ...
        if !Arc::ptr_eq(&current, &self.snapshot) {
            return Err(invalid("quill.publication", "changed", "Quill published another snapshot; select and admit a new native/lexical bundle"));
        }
        Ok(())
    }
}
...
impl LexicalRead for FencedQuillRead {
    fn search<'a>(&'a self, cx: &'a Cx, text: &'a str, limit: usize) -> SearchFuture<'a, Vec<ScoredResult>> {
        Box::pin(async move {
            let batch = self.search_candidates(cx, text, limit).await?;
            let mut results = batch.results().to_vec();
            self.hydrate_candidates(cx, batch.context(), &mut results).await?;
            Ok(results)
        })
    }
    ...
    fn hydrate_candidates<'a>(&'a self, cx: &'a Cx, context: Option<&'a LexicalHydrationContext>, results: &'a mut [ScoredResult]) -> SearchFuture<'a, ()> {
        Box::pin(async move {
    ...
    fn doc_count(&self) -> SearchResult<usize> {
        ...
        usize::try_from(self.snapshot.live_doc_count()).map_err(|_| {
            invalid("quill.doc_count", "overflow", "lexical live count does not fit usize")
        })
    }
}
...
mod tests {
    ...
    use frankensearch_core::generation::{ArtifactGenerationIdentityV1, CommitRange, EmbeddingIdentityBundleV1, QuantizationFormat};
    ...
    impl Provider {
        fn new() -> Self {
            Self { identity: EmbeddingIdentityBundleV1::explicit_test_model("quill-native", 2), ready: AtomicBool::new(true), calls: AtomicUsize::new(0) }
        }
    }
    ...
    impl Embedder for Provider {
        ...
        fn embed_bound<'a>(&'a self, _cx: &'a Cx, _text: &'a str) -> SearchFuture<'a, IdentityBoundEmbedding> {
            Box::pin(async move {
                ...
                poll_fn(|_| if self.ready.load(Ordering::SeqCst) { Poll::Ready(()) } else { Poll::Pending }).await;
    ...
    fn config() -> QuillConfig {
        QuillConfig { deterministic_ingest: true, max_ingest_shards: 1, ..QuillConfig::default() }
    }
    ...
    async fn fixture(cx: &Cx, provider: &Provider, vector_ids: &[&str]) -> Fixture {
        ...
        let receipt = snapshot.keeper_snapshot().exact_lexical_component_receipt(source_cut()).unwrap();
        let fast = vector_index(cx, provider, directory.path(), vector_ids, 7);
        Fixture { _directory: directory, lexical, fast, receipt }
    }

    fn vector_index(cx: &Cx, provider: &Provider, directory: &std::path::Path, ids: &[&str], generation: u64) -> Arc<NativeAnnIndex> {
        ...
        let binding = FsviV2IdentityBinding::new(ArtifactGenerationIdentityV1::new(generation, [0x34; 16]).unwrap(), identity.freeze().unwrap()).unwrap();
        let mut writer = VectorIndex::create_v2(&path, binding.clone()).unwrap();
        for id in ids { writer.write_record(id, &[1.0, 0.0]).unwrap(); }
    ...
    impl Fixture {
        fn admit(&self, cx: &Cx) -> SearchResult<NativeQuillSnapshot> {
            NativeQuillSnapshot::admit(cx, Arc::clone(&self.fast), self.fast.owner_witness(), Arc::clone(&self.lexical), &self.receipt, source_cut())
        }
    }
    ...
    fn real_quill_and_native_receipts_join_and_search_without_reopening() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            assert_eq!(view.vector_receipt().docset_digest, view.lexical_receipt().docset_digest);
            let hits = view.search_text(&cx, &provider, "common", 2).await.unwrap();
            assert_eq!(hits.len(), 2);
            assert!(hits.iter().all(|hit| hit.lexical_score.is_some() && hit.metadata.is_some()));
            assert_eq!(provider.calls.load(Ordering::SeqCst), 1);
            let cloned = view.clone();
            assert!(Arc::ptr_eq(view.lexical_snapshot(), cloned.lexical_snapshot()));
        });
    }
    ...
    fn same_count_different_document_sets_are_rejected() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            assert!(matches!(fixture.admit(&cx), Err(SearchError::InvalidConfig { field, .. }) if field == "native_ann.quill.docset"));
    ...
    fn complete_native_witness_not_only_generation_or_docset_is_checked() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            assert!(matches!(NativeQuillSnapshot::admit(&cx, Arc::clone(&fixture.fast), &expected, Arc::clone(&fixture.lexical), &fixture.receipt, source_cut()), Err(SearchError::InvalidConfig { field, .. }) if field == "native_ann.quill.fast"));
    ...
    fn lexical_byte_receipt_role_and_checkpoint_cannot_be_substituted() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            for case in 0..3 {
                ...
                assert!(matches!(NativeQuillSnapshot::admit(&cx, Arc::clone(&fixture.fast), fixture.fast.owner_witness(), Arc::clone(&fixture.lexical), &expected, cut), Err(SearchError::InvalidConfig { field, .. }) if field == "native_ann.quill.lexical"));
            }
            assert_eq!(provider.calls.load(Ordering::SeqCst), 0);
        });
    }
    ...
    fn unsealed_delta_is_not_authenticated_by_a_keeper_only_receipt() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            let provider = Provider::new();
            let fixture = fixture(&cx, &provider, &["alpha", "beta"]).await;
            fixture.lexical.upsert_documents(&cx, &[IndexableDocument::new("new", "common new")]).await.unwrap();
            assert!(fixture.lexical.search_snapshot().unwrap().delta_count() > 0);
            assert!(matches!(fixture.admit(&cx), Err(SearchError::InvalidConfig { value, .. }) if value == "unsealed-delta"));
        });
    }
    ...
    fn new_queries_refuse_a_changed_publication_even_when_k_is_zero() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            let fixture = fixture(&cx, &provider, &["alpha", "beta"]).await;
            let view = fixture.admit(&cx).unwrap();
            fixture.lexical.upsert_documents(&cx, &[IndexableDocument::new("alpha", "common changed")]).await.unwrap();
            for k in [0, 2] {
                assert!(matches!(view.search_text(&cx, &provider, "common", k).await, Err(SearchError::InvalidConfig { field, .. }) if field == "native_ann.quill.publication"));
            }
            assert!(view.search_progressive(&cx, &provider, "common", 2).is_err());
            assert_eq!(provider.calls.load(Ordering::SeqCst), 0);
        });
    }
    ...
    fn admitted_batch_finishes_with_old_metadata_after_writer_publication() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            fixture.lexical.upsert_documents(&cx, &[
                IndexableDocument::new("alpha", "common changed alpha").with_metadata("version", "new"),
                IndexableDocument::new("beta", "common changed beta").with_metadata("version", "new"),
            ]).await.unwrap();
            assert!(!Arc::ptr_eq(view.lexical_snapshot(), &fixture.lexical.search_snapshot().unwrap()));
    ...
    fn cancellation_and_noops_do_not_start_inference() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            let fixture = fixture(&cx, &provider, &["alpha", "beta"]).await;
            let view = fixture.admit(&cx).unwrap();
            assert!(view.search_text(&cx, &provider, "common", 0).await.unwrap().is_empty());
            cx.cancel_with(asupersync::CancelKind::User, Some("quill composition cancelled"));
