...
use crate::{Cx, Embedder, LexicalRead, ScoreSource, ScoredResult, SearchError, SearchResult, VectorHit};
...
impl NativeShardSet {
    ...
    pub async fn search_hybrid_quality_text(
        ...
        let mut results = self.search_hybrid_text(cx, embedder, lexical, text, k).await?;
    ...
    pub fn search_hybrid_progressive<'a>(
        ...
        Ok(NativeShardedProgressiveSearch {
            fast: self, cx, fast_embedder, quality, lexical, text, k,
            budget: candidate_budget(k)?, reranker: None, state: State::Initial,
        })
    }
}
...
enum State { Initial, Refine(Pending), Rerank(PendingRerank), Done }
...
impl<'a> NativeShardedProgressiveSearch<'a> {
    ...
    pub fn with_reranker(
        ...
        if !matches!(self.state, State::Initial) || top_k == 0 {
            return Err(invalid(
                "shards.rerank.configuration", "started-or-empty-window",
    ...
    #[must_use]
    pub const fn is_finished(&self) -> bool { matches!(self.state, State::Done) }
    ...
    pub async fn next_phase(&mut self) -> SearchResult<Option<NativeShardedSearchPhase>> {
        match std::mem::replace(&mut self.state, State::Done) {
            ...
            State::Refine(pending) => {
                match self.refine(&pending).await {
                    Ok((results, candidates)) => {
                        let results = self.finish_retrieval(results, candidates, pending.batch);
                        Ok(Some(NativeShardedSearchPhase::Refined { results, candidates }))
                    }
                    Err(error) => {
                        checkpoint(self.cx, "native_ann.shards.refinement_failure")?;
                        if matches!(error, SearchError::Cancelled { .. }) { return Err(error); }
                        ...
                        let initial_results = self.finish_retrieval(
                            pending.initial, candidates, pending.batch,
                        );
                        Ok(Some(NativeShardedSearchPhase::RefinementFailed { initial_results, error }))
                    }
                }
            }
    ...
    async fn initial(&mut self) -> SearchResult<NativeShardedSearchPhase> {
        ...
        if self.k == 0 {
            return Ok(NativeShardedSearchPhase::Initial {
                results: Vec::new(), candidates: NativePhaseCandidates::default(),
            });
        }
        let (fast, batch) = join_sources(
            self.cx,
            self.fast.search_text(self.cx, self.fast_embedder, self.text, self.budget, None),
            checked_lexical(self.cx, self.lexical, self.text, self.budget),
        ).await?;
        let candidates = NativePhaseCandidates { fast: fast.len(), quality: 0, lexical: batch.results().len() };
        let mut results = finish(self.cx, self.lexical, &batch, &fast, &[], self.result_window(), is_hash, false).await?;
        checkpoint(self.cx, "native_ann.shards.initial_complete")?;
        if self.quality.is_some() {
            self.state = State::Refine(Pending { fast, batch, initial: results.clone() });
            results.truncate(self.k);
        } else {
            results = self.finish_retrieval(results, candidates, batch);
        }
        Ok(NativeShardedSearchPhase::Initial { results, candidates })
    }

    async fn refine(&self, pending: &Pending) -> SearchResult<(Vec<NativeShardedResult>, NativePhaseCandidates)> {
        let is_hash = admit_tiers(self.cx, self.fast, self.fast_embedder, self.quality)?;
        let (quality, embedder) = self.quality.ok_or_else(|| {
            invalid("shards.quality", "missing", "refinement requires its admitted quality tier")
        })?;
        let quality = quality.search_text(self.cx, embedder, self.text, self.budget, None).await?;
        let candidates = NativePhaseCandidates { fast: pending.fast.len(), quality: quality.len(), lexical: pending.batch.results().len() };
        let results = finish(self.cx, self.lexical, &pending.batch, &pending.fast, &quality, self.result_window(), is_hash, true).await?;
    ...
    async fn rerank(&self, pending: PendingRerank) -> SearchResult<Option<NativeShardedSearchPhase>> {
        let request = self.reranker.as_ref().ok_or_else(|| {
            invalid("shards.rerank.configuration", "missing", "a queued rerank must retain its provider")
        })?;
        match request.plan(self.cx, self.text, pending.results.iter().map(|hit| &hit.result)).await {
            Ok(Some(plan)) => {
                ...
                Ok(Some(NativeShardedSearchPhase::Reranked { results, candidates: pending.candidates, evaluated }))
            }
            Ok(None) => Ok(None),
            Err(error) => {
                ...
                Ok(Some(NativeShardedSearchPhase::RerankFailed { previous_results, error }))
            }
        }
    }
}
...
fn candidate_budget(k: usize) -> SearchResult<usize> {
    k.checked_mul(3).ok_or_else(|| invalid(
        "shards.candidate_budget", "overflow", "three times the result count must fit usize",
    ))
}
...
fn admit_tiers(
    ...
    let first = fast.shard(0).ok_or_else(|| invalid("shards.inventory", "empty", "missing admitted fast identity"))?;
    if let Some((quality, embedder)) = quality {
        let quality = quality.shard(0).ok_or_else(|| invalid("shards.inventory", "empty", "missing admitted quality identity"))?;
    ...
    Ok(matches!(identity.space.kind, EmbeddingSpaceKindV1::HashControl))
}
...
async fn finish(
    ...
    let mut fused = rrf_fuse_for_vector_lane(batch.results(), &vectors, k, 0, &RrfConfig::default(), is_hash);
    ...
    for hit in &mut fused { hit.semantic_index = None; }
    ...
    let quality: BTreeMap<_, _> = quality.iter().map(|hit| (hit.doc_id.as_str(), hit)).collect();
    ...
    for mut result in winners {
        ...
        result.index = None;
        result.fast_score = if is_hash { None } else { fast.map(|hit| hit.score) };
        result.quality_score = if is_hash { None } else { quality.map(|hit| hit.score) };
...
fn ranking_vectors(hits: &[NativeShardHit]) -> Vec<VectorHit> {
    hits.iter().map(|hit| VectorHit {
        doc_id: hit.doc_id.clone(), score: hit.score, index: hit.row.physical_row,
    }).collect()
}
...
mod tests {
    ...
    #[derive(Clone, Copy)]
    enum Reply { Ready, Pending, Cancelled, Foreign, Failed }
    struct Provider {
    ...
    impl Provider {
        fn new(name: &str, dimension: usize, semantic: bool) -> Self {
            let mut identity = EmbeddingIdentityBundleV1::explicit_test_model(name, u32::try_from(dimension).unwrap());
            if semantic {
                ...
                identity.space.artifacts = vec![EmbeddingArtifactIdentityV1 { role: "weights".to_owned(), sha256: "b".repeat(64), size: 1 }];
                identity.producer.space_fingerprint = identity.space.fingerprint();
            }
            identity.validate().unwrap();
            let mut values = vec![0.0; dimension];
            values[0] = 1.0;
            Self { identity, values, calls: AtomicUsize::new(0), drops: AtomicUsize::new(0), reply: Reply::Ready }
        }
    }
    ...
    impl Embedder for Provider {
        ...
        fn embed_bound<'a>(&'a self, _cx: &'a Cx, _text: &'a str) -> SearchFuture<'a, IdentityBoundEmbedding> {
            Box::pin(async move {
                ...
                match self.reply {
                    ...
                    Reply::Cancelled => return Err(SearchError::Cancelled { phase: "test.sharded_provider".to_owned(), reason: "cancelled".to_owned() }),
                    Reply::Failed => return Err(invalid("test.sharded_provider", "failed", "provider failed")),
                    Reply::Foreign => identity.producer.backend = "foreign-response".to_owned(),
                }
                Ok(IdentityBoundEmbedding { values: self.values.clone(), identity })
            })
        }
        ...
        fn dimension(&self) -> usize { self.values.len() }
        fn is_ready(&self) -> bool { true }
        fn is_semantic(&self) -> bool { matches!(self.identity.space.kind, EmbeddingSpaceKindV1::Semantic) }
        fn category(&self) -> ModelCategory { if self.is_semantic() { ModelCategory::TransformerEmbedder } else { ModelCategory::HashEmbedder } }
    }
    ...
    impl Lexical {
        fn new(ids: &[&'static str]) -> Self {
            Self { ids: ids.to_vec(), calls: AtomicUsize::new(0), hydrations: AtomicUsize::new(0), current_generation: AtomicUsize::new(7), snapshot: Arc::new(7) }
        }
    }
    impl LexicalRead for Lexical {
        fn search<'a>(&'a self, _cx: &'a Cx, _text: &'a str, limit: usize) -> SearchFuture<'a, Vec<ScoredResult>> {
            Box::pin(async move {
                self.calls.fetch_add(1, Ordering::SeqCst);
                Ok(self.ids.iter().take(limit).map(|id| ScoredResult {
                    doc_id: (*id).into(), score: 10.0, source: ScoreSource::Lexical,
                    index: None, fast_score: None, quality_score: None, lexical_score: Some(10.0),
                    rerank_score: None, explanation: None, metadata: None,
                }).collect())
            })
        }
        fn search_candidates<'a>(&'a self, cx: &'a Cx, text: &'a str, limit: usize) -> SearchFuture<'a, LexicalCandidateBatch> {
            Box::pin(async move {
                Ok(LexicalCandidateBatch::deferred(self.search(cx, text, limit).await?,
                    LexicalHydrationContext::new("sharded-test", Box::new(Arc::clone(&self.snapshot)))))
            })
        }
        fn hydrate_candidates<'a>(&'a self, _cx: &'a Cx, context: Option<&'a LexicalHydrationContext>, results: &'a mut [ScoredResult]) -> SearchFuture<'a, ()> {
            Box::pin(async move {
                self.hydrations.fetch_add(1, Ordering::SeqCst);
                let pin = context.and_then(LexicalHydrationContext::downcast_ref::<Arc<usize>>).unwrap();
        ...
        fn doc_count(&self) -> SearchResult<usize> { Ok(self.ids.len()) }
    }

    fn partition(cx: &Cx, provider: &Provider, rows: &[(&str, &[f32])], generation: u64, ann: bool) -> Arc<NativeAnnIndex> {
        ...
        let binding = FsviV2IdentityBinding::new(ArtifactGenerationIdentityV1::new(generation, [0x72; 16]).unwrap(), bundle.freeze().unwrap()).unwrap();
        ...
        Arc::new(if ann { NativeAnnIndex::build(cx, owner, HnswParams::default(), 7).unwrap() } else { NativeAnnIndex::exact(cx, owner).unwrap() })
    }
    fn set(cx: &Cx, parts: Vec<Arc<NativeAnnIndex>>) -> NativeShardSet {
        let expected: Vec<_> = parts.iter().map(|part| part.owner_witness().clone()).collect();
        NativeShardSet::admit(cx, &expected, parts).unwrap()
    }
    fn fast_set(cx: &Cx, provider: &Provider) -> NativeShardSet {
        set(cx, vec![
            partition(cx, provider, &[("z-fast", &[1.0, 0.0]), ("mid-fast", &[0.9, 0.1])], 1, true),
            partition(cx, provider, &[("near-fast", &[0.8, 0.2]), ("a-late", &[0.0, 1.0])], 1, false),
        ])
    }
    fn quality_set(cx: &Cx, provider: &Provider) -> NativeShardSet {
        set(cx, vec![
            partition(cx, provider, &[("q-other", &[0.0, 1.0, 0.0]), ("z-fast", &[0.75, 0.0, 0.0])], 1, false),
    ...
    fn progressive_quality_finds_a_document_outside_every_fast_candidate_shard() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            let mut stream = a.search_hybrid_progressive(&cx, &fast, Some((&b, &quality)), &lexical, "query", 1).unwrap();
            let NativeShardedSearchPhase::Initial { results, candidates } = stream.next_phase().await.unwrap().unwrap() else { panic!("initial"); };
            assert_eq!(results[0].result.doc_id, "z-fast");
            assert_eq!(candidates.fast, 3);
            assert_eq!(quality.calls.load(Ordering::SeqCst), 0);
            let State::Refine(pending) = &stream.state else { panic!("retained candidates"); };
            assert!(pending.fast.iter().all(|hit| hit.doc_id != "a-late"));
            let NativeShardedSearchPhase::Refined { results, candidates } = stream.next_phase().await.unwrap().unwrap() else { panic!("refined"); };
            assert_eq!(results[0].result.doc_id, "a-late");
            assert_eq!(results[0].result.source, ScoreSource::SemanticQuality);
            assert_eq!(results[0].quality_row, Some(NativeShardRow { shard: 1, physical_row: 0 }));
    ...
    fn shared_winners_keep_distinct_fast_quality_locations_and_raw_scores() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            let hits = a.search_hybrid_refined_text(&cx, &fast, (&b, &quality), &lexical, "query", 4).await.unwrap();
            let shared = hits.iter().find(|hit| hit.result.doc_id == "z-fast").unwrap();
            assert_eq!(shared.fast_row, Some(NativeShardRow { shard: 0, physical_row: 0 }));
            assert_eq!(shared.quality_row, Some(NativeShardRow { shard: 0, physical_row: 1 }));
            assert_eq!((shared.result.fast_score, shared.result.quality_score), (Some(1.0), Some(0.75)));
    ...
    fn quality_primary_requires_no_fast_shards_and_preserves_quality_coordinates() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            let quality = Provider::new("quality", 3, true);
            let b = quality_set(&cx, &quality);
            let hits = b.search_hybrid_quality_text(&cx, &quality, &Lexical::new(&[]), "query", 1).await.unwrap();
            ...
            assert_eq!(hits[0].quality_row, Some(NativeShardRow { shard: 1, physical_row: 0 }));
    ...
    fn both_phases_hydrate_the_original_lexical_snapshot_without_requery() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            let mut stream = a.search_hybrid_progressive(&cx, &fast, Some((&b, &quality)), &lexical, "query", 2).unwrap();
            ...
            let NativeShardedSearchPhase::Refined { results, .. } = stream.next_phase().await.unwrap().unwrap() else { panic!("refined"); };
            assert!(results.iter().all(|hit| hit.result.metadata.as_deref().unwrap()["generation"] == 7));
            assert_eq!(lexical.calls.load(Ordering::SeqCst), 1);
            assert_eq!(lexical.hydrations.load(Ordering::SeqCst), 2);
            assert_eq!(Arc::strong_count(&lexical.snapshot), 1);
            let eager = a.search_hybrid_refined_text(&cx, &fast, (&b, &quality), &lexical, "query", 2).await.unwrap();
            assert_eq!(serde_json::to_value(results.iter().map(|hit| &hit.result).collect::<Vec<_>>()).unwrap(), serde_json::to_value(eager.iter().map(|hit| &hit.result).collect::<Vec<_>>()).unwrap());
            assert_eq!(results.iter().map(|hit| (hit.fast_row, hit.quality_row)).collect::<Vec<_>>(), eager.iter().map(|hit| (hit.fast_row, hit.quality_row)).collect::<Vec<_>>());
        });
    }
    ...
    fn abandoning_quality_starts_no_work_or_drops_pending_work_without_retry() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            for poll_quality in [false, true] {
                ...
                drop(stream);
                assert_eq!(quality.calls.load(Ordering::SeqCst), usize::from(poll_quality));
                assert_eq!(quality.drops.load(Ordering::SeqCst), usize::from(poll_quality));
                assert_eq!(Arc::strong_count(&lexical.snapshot), 1);
            }
        });
    }
    ...
    fn quality_errors_preserve_initial_results_but_cancellation_is_terminal() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            for reply in [Reply::Failed, Reply::Foreign, Reply::Cancelled] {
                ...
                let lexical = Lexical::new(&["z-fast"]);
                let mut stream = a.search_hybrid_progressive(&cx, &fast, Some((&b, &quality)), &lexical, "query", 1).unwrap();
                let NativeShardedSearchPhase::Initial { results, .. } = stream.next_phase().await.unwrap().unwrap() else { panic!("initial"); };
                let expected = serde_json::to_value(&results[0].result).unwrap();
                let next = stream.next_phase().await;
                if matches!(reply, Reply::Cancelled) {
                    assert!(matches!(next, Err(SearchError::Cancelled { ref phase, .. }) if phase == "test.sharded_provider"));
                } else {
                    let NativeShardedSearchPhase::RefinementFailed { initial_results, error } = next.unwrap().unwrap() else { panic!("explicit failure"); };
                    assert_eq!(serde_json::to_value(&initial_results[0].result).unwrap(), expected);
    ...
    fn tier_generation_mismatch_and_noop_identity_errors_precede_all_provider_work() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            let lexical = Lexical::new(&[]);
            for k in [0, 1] {
                assert!(matches!(a.search_hybrid_refined_text(&cx, &fast, (&b, &quality), &lexical, "query", k).await, Err(SearchError::InvalidConfig { ref field, .. }) if field == "native_ann.tier_generation"));
            }
            assert!(a.search_hybrid_text(&cx, &quality, &lexical, "query", 0).await.is_err());
    ...
    fn hash_shards_keep_nonsemantic_labels_but_unambiguous_tier_coordinates() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            let a = fast_set(&cx, &fast);
            let b = quality_set(&cx, &quality);
            let hits = a.search_hybrid_refined_text(&cx, &fast, (&b, &quality), &Lexical::new(&[]), "query", 1).await.unwrap();
            assert_eq!(hits[0].result.source, ScoreSource::HashControl);
            assert!(hits[0].result.fast_score.is_none() && hits[0].result.quality_score.is_none() && hits[0].result.index.is_none());
            assert_eq!(hits[0].quality_row, Some(NativeShardRow { shard: 1, physical_row: 0 }));
        });
    }
    ...
    fn empty_fast_shards_still_allow_quality_and_zero_k_finishes_without_work() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            let b = quality_set(&cx, &quality);
            let lexical = Lexical::new(&[]);
            let mut empty = a.search_hybrid_progressive(&cx, &fast, Some((&b, &quality)), &lexical, "query", 0).unwrap();
            assert!(matches!(empty.next_phase().await.unwrap(), Some(NativeShardedSearchPhase::Initial { results, .. }) if results.is_empty()));
            assert!(empty.next_phase().await.unwrap().is_none());
            assert_eq!(lexical.calls.load(Ordering::SeqCst), 0);
            let mut stream = a.search_hybrid_progressive(&cx, &fast, Some((&b, &quality)), &lexical, "query", 1).unwrap();
            drop(stream.next_phase()); // Unpolled futures do not consume a phase.
            assert!(!stream.is_finished());
            assert!(matches!(stream.next_phase().await.unwrap(), Some(NativeShardedSearchPhase::Initial { results, .. }) if results.is_empty()));
            assert!(matches!(stream.next_phase().await.unwrap(), Some(NativeShardedSearchPhase::Refined { results, .. }) if results[0].result.doc_id == "a-late"));
            assert_eq!(fast.calls.load(Ordering::SeqCst), 0);
            assert_eq!(quality.calls.load(Ordering::SeqCst), 1);
        });
    }
    ...
    #[derive(Clone, Copy)]
    enum RerankReply { Ready, Pending, Invalid, Failed, Cancelled, CancelWithScores, Unavailable }
    ...
    impl CrossEncoder {
        fn new(winner: &'static str, reply: RerankReply) -> Self {
            Self { winner, reply, calls: AtomicUsize::new(0), drops: AtomicUsize::new(0) }
        }
    }
    ...
    impl Reranker for CrossEncoder {
            ...
            Box::pin(async move {
                ...
                let mut scores: Vec<_> = documents.iter().enumerate().map(|(rank, doc)| {
                    ...
                    frankensearch_core::traits::RerankScore {
                        ...
                        score: if doc.doc_id == self.winner { 0.95 } else { 0.05 },
                        original_rank: rank,
                        raw_logit: None,
                    }
                }).collect();
                match self.reply {
                    ...
                    RerankReply::Failed => return Err(invalid("test.sharded_reranker", "failed", "model failed")),
                    RerankReply::Cancelled => return Err(SearchError::Cancelled {
                        phase: "test.sharded_reranker".to_owned(), reason: "model cancelled".to_owned(),
                    }),
                    RerankReply::CancelWithScores => {
                        cx.cancel_with(asupersync::CancelKind::User, Some("cancel with sharded rerank scores"));
                    }
        ...
        fn id(&self) -> &str { "sharded-test-cross-encoder" }
        fn model_name(&self) -> &str { self.id() }
    ...
    fn rerank_promotes_an_undisplayed_second_shard_hit_with_one_global_model_call() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            let mut stream = a.search_hybrid_progressive(&cx, &fast, None, &lexical, "query", 1)
                .unwrap().with_reranker(&model, &text, 4).unwrap();
            let NativeShardedSearchPhase::Initial { results, .. } = stream.next_phase().await.unwrap().unwrap() else { panic!("initial") };
            assert_eq!(results[0].result.doc_id, "z-fast");
            assert_eq!(model.calls.load(Ordering::SeqCst), 0);
            assert_eq!(text_calls.load(Ordering::SeqCst), 0);
            let State::Rerank(pending) = &stream.state else { panic!("retained final window") };
            let original = pending.results.iter().find(|hit| hit.result.doc_id == "a-late").unwrap().clone();
            lexical.current_generation.store(99, Ordering::SeqCst);
            let NativeShardedSearchPhase::Reranked { results, evaluated, candidates } = stream.next_phase().await.unwrap().unwrap() else { panic!("reranked") };
            assert_eq!(results.len(), 1);
            let winner = &results[0];
            assert_eq!(winner.result.doc_id, "a-late");
            assert_eq!(winner.fast_row, Some(NativeShardRow { shard: 1, physical_row: 1 }));
            ...
            assert_eq!(winner.result.score.to_bits(), original.result.score.to_bits());
            assert_eq!(winner.result.fast_score, original.result.fast_score);
            assert!(Arc::ptr_eq(winner.result.metadata.as_ref().unwrap(), original.result.metadata.as_ref().unwrap()));
            assert_eq!(winner.result.metadata.as_deref().unwrap()["generation"], 7);
            assert_eq!(evaluated, 4);
            assert_eq!(candidates, NativePhaseCandidates { fast: 4, quality: 0, lexical: 4 });
    ...
    fn final_permutation_preserves_distinct_tier_rows_and_quality_only_provenance() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            for target in ["z-fast", "a-late", "q-other"] {
                ...
                let a = fast_set(&cx, &fast);
                let b = quality_set(&cx, &quality);
                let lexical = Lexical::new(&["z-fast", "mid-fast", "near-fast", "a-late", "q-other"]);
                let model = CrossEncoder::new(target, RerankReply::Ready);
                let mut stream = a.search_hybrid_progressive(&cx, &fast, Some((&b, &quality)), &lexical, "query", 1)
                    .unwrap().with_reranker(&model, &source_text, 5).unwrap();
                assert!(matches!(stream.next_phase().await.unwrap(), Some(NativeShardedSearchPhase::Initial { .. })));
                assert_eq!(quality.calls.load(Ordering::SeqCst), 0);
                assert!(matches!(stream.next_phase().await.unwrap(), Some(NativeShardedSearchPhase::Refined { .. })));
                assert_eq!(model.calls.load(Ordering::SeqCst), 0);
                let State::Rerank(pending) = &stream.state else { panic!("retained refined window") };
                let original = pending.results.iter().find(|hit| hit.result.doc_id == target).unwrap().clone();
                let NativeShardedSearchPhase::Reranked { results, evaluated, .. } = stream.next_phase().await.unwrap().unwrap() else { panic!("final") };
                assert_eq!(evaluated, 5);
                let winner = &results[0];
                assert_eq!(winner.result.doc_id, target);
                assert_eq!((winner.fast_row, winner.quality_row), (original.fast_row, original.quality_row));
                assert_eq!(winner.result.fast_score, original.result.fast_score);
                ...
                match target {
                    "z-fast" => {
                        assert_eq!(winner.fast_row, Some(NativeShardRow { shard: 0, physical_row: 0 }));
                        assert_eq!(winner.quality_row, Some(NativeShardRow { shard: 0, physical_row: 1 }));
                        assert_eq!((winner.result.fast_score, winner.result.quality_score), (Some(1.0), Some(0.75)));
                    }
                    "a-late" => {
                        assert_eq!(winner.fast_row, Some(NativeShardRow { shard: 1, physical_row: 1 }));
                        assert_eq!(winner.quality_row, Some(NativeShardRow { shard: 1, physical_row: 0 }));
                    }
                    _ => {
                        assert!(winner.fast_row.is_none() && winner.result.fast_score.is_none());
                        assert_eq!(winner.quality_row, Some(NativeShardRow { shard: 0, physical_row: 0 }));
    ...
    fn quality_failure_keeps_its_page_and_allows_the_requested_final_stage() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            let NativeShardedSearchPhase::Initial { results, .. } = stream.next_phase().await.unwrap().unwrap() else { panic!("initial") };
            let expected = serde_json::to_value(&results[0].result).unwrap();
            let NativeShardedSearchPhase::RefinementFailed { initial_results, .. } = stream.next_phase().await.unwrap().unwrap() else { panic!("quality failure is explicit") };
            assert_eq!(serde_json::to_value(&initial_results[0].result).unwrap(), expected);
            assert_eq!(initial_results[0].fast_row, results[0].fast_row);
            assert_eq!(model.calls.load(Ordering::SeqCst), 0);
            let NativeShardedSearchPhase::Reranked { results, .. } = stream.next_phase().await.unwrap().unwrap() else { panic!("independent final stage") };
            assert_eq!(results[0].result.doc_id, "near-fast");
            assert_eq!(results[0].fast_row, Some(NativeShardRow { shard: 1, physical_row: 0 }));
    ...
    fn invalid_model_response_keeps_the_whole_previous_envelope_unchanged() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            let fast = Provider::new("fast", 2, true);
            let a = fast_set(&cx, &fast);
            for reply in [RerankReply::Invalid, RerankReply::Failed, RerankReply::Unavailable] {
                let lexical = Lexical::new(&["z-fast", "mid-fast", "near-fast", "a-late"]);
                let model = CrossEncoder::new("a-late", reply);
                let mut stream = a.search_hybrid_progressive(&cx, &fast, None, &lexical, "query", 2)
                    .unwrap().with_reranker(&model, &source_text, 4).unwrap();
                let NativeShardedSearchPhase::Initial { results, .. } = stream.next_phase().await.unwrap().unwrap() else { panic!("initial") };
                let NativeShardedSearchPhase::RerankFailed { previous_results, .. } = stream.next_phase().await.unwrap().unwrap() else { panic!("atomic failure") };
                assert_eq!(previous_results.len(), results.len());
                for (previous, initial) in previous_results.iter().zip(&results) {
                    assert_eq!(serde_json::to_value(&previous.result).unwrap(), serde_json::to_value(&initial.result).unwrap());
                    assert_eq!((previous.fast_row, previous.quality_row), (initial.fast_row, initial.quality_row));
                    assert!(Arc::ptr_eq(previous.result.metadata.as_ref().unwrap(), initial.result.metadata.as_ref().unwrap()));
                }
                assert_eq!(model.calls.load(Ordering::SeqCst), usize::from(!matches!(reply, RerankReply::Unavailable)));
                assert_eq!(Arc::strong_count(&lexical.snapshot), 1);
                assert!(stream.is_finished() && stream.next_phase().await.unwrap().is_none());
            }
        });
    ...
    fn missing_text_and_hash_tiers_do_not_invent_semantic_scores_or_row_locations() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            let lexical = Lexical::new(&[]);
            let model = CrossEncoder::new("near-fast", RerankReply::Ready);
            let only_near = |id: &str| (id == "near-fast").then(|| format!("generation-seven:{id}"));
            let mut stream = a.search_hybrid_progressive(&cx, &fast, None, &lexical, "query", 4)
                .unwrap().with_reranker(&model, &only_near, 3).unwrap();
            assert!(stream.next_phase().await.unwrap().is_some());
            let NativeShardedSearchPhase::Reranked { results, evaluated, .. } = stream.next_phase().await.unwrap().unwrap() else { panic!("one scored candidate") };
            assert_eq!(evaluated, 1);
            assert_eq!(results[0].result.doc_id, "near-fast");
            assert_eq!(results[0].fast_row, Some(NativeShardRow { shard: 1, physical_row: 0 }));
            assert_eq!(results[0].result.source, ScoreSource::Reranked);
            assert!(results.iter().all(|hit| hit.result.fast_score.is_none() && hit.result.quality_score.is_none()
                && hit.result.index.is_none() && hit.quality_row.is_none()));
            assert_eq!(results[1..].iter().map(|hit| hit.result.doc_id.as_str()).collect::<Vec<_>>(), ["z-fast", "mid-fast", "a-late"]);
            assert!(results[1..].iter().all(|hit| hit.result.source == ScoreSource::HashControl && hit.result.rerank_score.is_none()));
            let no_text = |_: &str| None;
            let mut empty = a.search_hybrid_progressive(&cx, &fast, None, &lexical, "query", 1)
    ...
    fn sharded_rerank_noops_and_invalid_budgets_start_no_provider_work() {
        asupersync::test_utils::run_test_with_cx(|cx| async move {
            ...
            for budget in [0, usize::MAX] {
                assert!(a.search_hybrid_progressive(&cx, &fast, None, &lexical, "query", 1)
                    .unwrap().with_reranker(&model, &source_text, budget).is_err());
            }
            let mut empty = a.search_hybrid_progressive(&cx, &fast, None, &lexical, "query", 0)
                .unwrap().with_reranker(&model, &source_text, 4).unwrap();
            assert!(matches!(empty.next_phase().await.unwrap(), Some(NativeShardedSearchPhase::Initial { results, .. }) if results.is_empty()));
            ...
            assert_eq!(lexical.calls.load(Ordering::SeqCst), 0);
            assert_eq!(model.calls.load(Ordering::SeqCst), 0);
            let mut late = a.search_hybrid_progressive(&cx, &fast, None, &lexical, "query", 1).unwrap();
