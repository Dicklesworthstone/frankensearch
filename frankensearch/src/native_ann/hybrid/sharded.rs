...
impl NativeShardSet {
    ...
    pub fn search_hybrid_progressive<'a>(
        ...
        Ok(NativeShardedProgressiveSearch {
            ...
            budget: candidate_budget(k)?, state: State::Initial,
        })
    }
}
...
enum State { Initial, Refine(Pending), Done }
...
impl NativeShardedProgressiveSearch<'_> {
        ...
        match std::mem::replace(&mut self.state, State::Done) {
            ...
            State::Refine(pending) => {
                match self.refine(&pending).await {
                    Ok((results, candidates)) => Ok(Some(NativeShardedSearchPhase::Refined { results, candidates })),
                    Err(error) => {
                        ...
                        Ok(Some(NativeShardedSearchPhase::RefinementFailed {
                            initial_results: pending.initial, error,
                        }))
                    }
                }
            }
    ...
    async fn initial(&mut self) -> SearchResult<NativeShardedSearchPhase> {
        ...
        let results = finish(self.cx, self.lexical, &batch, &fast, &[], self.k, is_hash, false).await?;
    ...
    async fn refine(&self, pending: &Pending) -> SearchResult<(Vec<NativeShardedResult>, NativePhaseCandidates)> {
        ...
        let results = finish(self.cx, self.lexical, &pending.batch, &pending.fast, &quality, self.k, is_hash, true).await?;
